<div class="space-y-6" wire:poll.15s>
                    ...
                    <div class="p-2.5 flex-1 overflow-y-auto space-y-2 custom-scrollbar">
                        ...
                        @else
                            @foreach ($dayApps as $app)
                                ...
                                @endphp
                                <div class="p-2.5 rounded-lg border flex flex-col gap-1.5" style="background: {{ $style['bg'] }}; border-color: {{ $style['border'] }};">
    ...
    {{-- Quick Action Cards --}}
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-{{ auth()->user()->role === 'admin' ? '4' : '6' }} gap-4">
    ...
    <div class="fixed inset-0 z-50 p-4 flex items-center justify-center transition-all duration-300 backdrop-blur-md bg-[#060d1a]/60 {{ $showModal ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none' }}">
        ...
        <div class="rounded-3xl shadow-2xl w-full max-w-4xl relative z-10 border overflow-hidden transform transition-all bg-[#0a1628] border-white/10 {{ $showModal ? 'scale-100' : 'scale-95' }}">
            @if ($selectedApp)
                <div class="px-8 py-6 border-b border-white/5 flex justify-between items-center bg-white/5">
                    <div class="flex items-center gap-3">
                        <span class="bg-cyan-500/20 text-cyan-400 font-mono text-xs font-black px-2 py-1 rounded border border-cyan-500/30 italic">#{{ $selectedApp->id }}</span>
    ...
    @if($role === 'cashier')
            ...
            ═══════════════════════════════════════════════════════ --}}
            <div class="p-5 rounded-2xl border relative overflow-hidden group shadow-2xl shadow-black/40"
                 style="background: rgba(255,255,255,0.06); backdrop-filter: blur(16px); border-color: rgba(255,255,255,0.10);">

                <div class="absolute top-0 right-0 p-8 opacity-5 mt-[-10px] mr-[-10px] transition-transform group-hover:scale-110 duration-700">
                    <svg class="w-40 h-40 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                </div>

                <div class="flex justify-between items-center relative z-10">
                    <div class="flex items-center gap-4">
                        <div class="w-12 h-12 rounded-xl bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center text-emerald-400 shadow-lg shadow-emerald-500/10 transition-transform group-hover:scale-105">
                        ...
                        <div>
                            <h2 class="text-xl font-bold text-white leading-none tracking-tight">Financial Overview</h2>
                            <p class="text-[8px] mt-1.5 font-medium uppercase tracking-[0.3em]" style="color: rgba(255,255,255,0.4);">Daily Revenue & Collection Metrics</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                {{-- Collected Today --}}
                <div class="p-4 rounded-xl border group transition-all duration-500 hover:scale-[1.02]"
                     style="background: rgba(16,185,129,0.04); border-color: rgba(16,185,129,0.15); box-shadow: 0 10px 40px rgba(0,0,0,0.2);">
                    <div class="flex justify-between items-start mb-4">
                        <div>
                            <h4 class="text-[8px] font-black text-emerald-400/80 uppercase tracking-[0.3em] mb-2">Collected Today</h4>
                            <div class="text-2xl font-black text-white tracking-tighter transition-transform group-hover:translate-x-1 duration-500">
                                ₱{{ number_format($stats['daily_collection'] ?? 0, 2) }}
                            </div>
                        </div>
                        <div class="p-2 rounded-xl bg-emerald-500/10 text-emerald-400 group-hover:rotate-12 transition-transform shadow-lg shadow-emerald-500/10">
                    ...
                    <p class="text-[8px] mt-2 font-bold text-white/20 uppercase tracking-[0.2em] italic">Confirmed Revenue</p>
                </div>
                ...
                <div class="p-4 rounded-xl border group transition-all duration-500 hover:scale-[1.02]"
                    ...
                    <div class="flex justify-between items-start mb-4">
                        <div>
                            <h4 class="text-[8px] font-black text-blue-400/80 uppercase tracking-[0.3em] mb-2">Receipts Today</h4>
                            <div class="text-2xl font-black text-white tracking-tighter transition-transform group-hover:translate-x-1 duration-500">
                                {{ $stats['transactions_today'] ?? 0 }}
                            </div>
                        </div>
                        <div class="p-2 rounded-xl bg-blue-500/10 text-blue-400 group-hover:rotate-12 transition-transform shadow-lg shadow-blue-500/10">
                    ...
                    <p class="text-[8px] mt-2 font-bold text-white/20 uppercase tracking-[0.2em] italic">Daily Transaction Volume</p>
                </div>
                ...
                <div class="p-4 rounded-xl border group transition-all duration-500 hover:scale-[1.02]"
                    ...
                    <div class="flex justify-between items-start mb-4">
                        <div>
                            <h4 class="text-[8px] font-black text-amber-400/80 uppercase tracking-[0.3em] mb-2">Verification Queue</h4>
                            <div class="text-2xl font-black text-white tracking-tighter transition-transform group-hover:translate-x-1 duration-500">
                                {{ $stats['pending_verifications'] ?? 0 }}
                            </div>
                        </div>
                        <div class="p-2 rounded-xl bg-amber-500/10 text-amber-400 group-hover:rotate-12 transition-transform shadow-lg shadow-amber-500/10">
                    ...
                    <p class="text-[8px] mt-2 font-bold text-white/20 uppercase tracking-[0.2em] italic">Pending Financial Audit</p>
                </div>
            </div>
                ...
                <div class="overflow-x-auto custom-scrollbar">
                    <table class="w-full text-left">
                        ...
                        <tbody class="divide-y divide-white/5">
                                @php $todayP = $paymentsToday ?? []; @endphp
                                @forelse($todayP as $payment)
                ...
                @else
                    <div class="overflow-x-auto custom-scrollbar">
                        <table class="w-full text-left">
                            ...
                            <tbody class="divide-y divide-white/5">
                             @php $yesterdayP = $paymentsYesterday ?? []; @endphp
                             @forelse($yesterdayP as $payment)
                                <tr class="hover:bg-white/[0.02] transition-colors group">
                                    <td class="py-5 px-6 font-mono text-xs text-white/30 italic group-hover:text-emerald-400 transition-colors">#{{ str_pad($payment->id, 6, '0', STR_PAD_LEFT) }}</td>
                                    <td class="py-5 px-6 font-bold text-white uppercase tracking-tight">{{ optional($payment->user)->name }}</td>
                                    <td class="py-5 px-6 text-xs text-white/40">{{ $payment->updated_at->format('H:i A') }}</td>
                                ...
                                <tr>
                                    <td colspan="5" class="py-24 text-center">
                                        <p class="text-xs font-black text-white/10 uppercase tracking-[0.4em] italic leading-loose">No activity detected for yesterday.</p>
                                    </td>
                                </tr>
                                @endforelse
                            </tbody>
