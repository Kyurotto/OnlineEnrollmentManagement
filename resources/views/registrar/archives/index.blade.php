<x-layouts.registrar title="Academic Archives">
        ...
        @if(!$selectedFolder)
            ...
            <div class="p-8 md:p-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                @forelse($folders as $folder)
                    <a href="{{ route('registrar.archives.index', ['selectedFolder' => $folder->semester_name . '|' . $folder->academic_year_name]) }}"
        ...
        @else
            ...
            <div class="p-8 md:p-10 border-b border-white/5 bg-white/[0.01] space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <form action="{{ route('registrar.archives.index') }}" method="GET" class="md:col-span-2 relative group">
                    ...
                    <select onchange="window.location.href=this.value" class="bg-white/[0.03] border border-white/10 rounded-2xl px-4 py-3 text-[10px] text-white/60 font-black uppercase tracking-widest focus:border-cyan-500/50 outline-none transition-all">
                        <option value="{{ route('registrar.archives.index', ['selectedFolder' => $selectedFolder, 'search' => $search, 'selectedCourse' => $selectedCourse, 'level' => '']) }}">ALL LEVELS</option>
                        <option value="{{ route('registrar.archives.index', ['selectedFolder' => $selectedFolder, 'search' => $search, 'selectedCourse' => $selectedCourse, 'level' => 'college']) }}" {{ $level === 'college' ? 'selected' : '' }}>COLLEGE</option>
                        <option value="{{ route('registrar.archives.index', ['selectedFolder' => $selectedFolder, 'search' => $search, 'selectedCourse' => $selectedCourse, 'level' => 'shs']) }}" {{ $level === 'shs' ? 'selected' : '' }}>SHS</option>
                    </select>

                    <select onchange="window.location.href=this.value" class="bg-white/[0.03] border border-white/10 rounded-2xl px-4 py-3 text-[10px] text-white/60 font-black uppercase tracking-widest focus:border-cyan-500/50 outline-none transition-all">
                        <option value="{{ route('registrar.archives.index', ['selectedFolder' => $selectedFolder, 'search' => $search, 'level' => $level, 'selectedCourse' => '']) }}">ALL COURSES</option>
                        @foreach($courses as $course)
                            <option value="{{ route('registrar.archives.index', ['selectedFolder' => $selectedFolder, 'search' => $search, 'level' => $level, 'selectedCourse' => $course->course_code]) }}" {{ $selectedCourse === $course->course_code ? 'selected' : '' }}>
                                {{ $course->course_code }}
                            </option>
                        @endforeach
                    </select>
                ...
                <table class="w-full text-left border-collapse font-bold">
                    <thead>
                        <tr class="text-[10px] text-white/20 uppercase tracking-[0.2em] bg-white/[0.02]">
                            ...
                            <th class="py-6 px-8">Academic Track</th>
                            ...
                            <tr class="hover:bg-white/[0.02] transition-colors group">
                                <td class="py-6 px-8">
                                    <div class="flex items-center gap-3">
                                        <div class="w-1 h-8 rounded-full {{ $app->status === 'Enrolled' ? 'bg-emerald-500' : 'bg-cyan-500/40' }}"></div>
                                        <span class="text-white/20 font-mono tracking-tighter">#{{ str_pad($app->id, 5, '0', STR_PAD_LEFT) }}</span>
                                    </div>
                                </td>
                                <td class="py-6 px-8">
                                ...
                                <td class="py-6 px-8 text-right">
                                    <div class="flex justify-end gap-2">
                                        <a href="{{ route('registrar.applications.show', $app->id) }}"
                                            class="inline-flex items-center gap-2 px-6 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white/40 hover:text-cyan-400 hover:border-cyan-500/30 transition-all text-[10px] font-black uppercase tracking-widest group/btn">
                                            VIEW DETAILS
