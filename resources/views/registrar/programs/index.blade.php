    ...
    <div class="space-y-12 animate-in fade-in duration-500">
        ...
        @if(session('success'))
            <div id="success-toast" class="fixed top-6 right-6 z-[60] animate-in slide-in-from-top-2 duration-300">
                <div
                    class="bg-emerald-500 text-black px-8 py-4 rounded-xl font-black uppercase text-xs tracking-widest shadow-2xl shadow-emerald-500/40 flex items-center gap-3 backdrop-blur-lg border border-emerald-400/20">
        ...
        @if(session('error'))
            <div
                class="bg-rose-500/10 border border-rose-500/20 text-rose-400 px-6 py-4 rounded-xl shadow-lg backdrop-blur-md flex items-center gap-3 font-bold animate-in fade-in duration-300">
        ...
        <section class="p-8 rounded-2xl bg-white/5 border border-white/10 shadow-2xl">
            <div class="flex justify-between items-center mb-10">
                <div>
                    ...
                    <p class="text-xs text-white/40 uppercase tracking-widest mt-2 font-bold">Undergraduate Degree
                        Courses</p>
                </div>
                <a href="{{ route('registrar.programs.index', ['showModal' => 'true', 'type' => 'program']) }}"
                    class="bg-blue-600 hover:bg-blue-500 text-white px-6 py-3 rounded-xl text-xs font-bold uppercase tracking-widest transition-all shadow-lg shadow-blue-500/20 active:scale-95 flex items-center gap-2">
            ...
            <div class="overflow-x-auto custom-scrollbar">
                <table class="w-full text-left font-medium text-sm">
                    <thead class="text-xs text-white/20 uppercase tracking-widest border-b border-white/5"
                    ...
                    <tbody class="divide-y divide-white/5">
                        @forelse($programs as $program)
                            <tr class="hover:bg-blue-500/[0.03] transition-all group">
                                <td class="py-5 px-8">
                                    <span
                                        class="text-blue-400 font-bold font-mono text-xs">{{ $program->course_code }}</span>
                                </td>
                                <td class="py-5 px-8">
                                    <span
                                        class="text-white font-bold group-hover:text-blue-300 transition-colors uppercase tracking-tight">{{ $program->course_name }}</span>
                                </td>
                                <td class="py-5 px-8">
                                    @php
                                        $finalDesc = $program->description;
                                        if (empty($finalDesc)) {
                                            $finalDesc = match (strtoupper($program->course_code)) {
                                                'ACT' => 'Associate in Computer Technology',
                                                'BSIS' => 'Bachelor of Science in Information Systems',
                                                'BTVTED' => 'Bachelor of Technical-Vocational Teacher Education',
                                                'DHRT' => 'Diploma in Hotel and Restaurant Technology',
                                    ...
                                    @endphp
                                    <span
                                        class="text-white/40 text-sm line-clamp-1 font-medium">{{ $finalDesc }}</span>
                                </td>
                                <td class="py-5 px-8 text-right whitespace-nowrap">
                                    <div class="flex justify-end gap-2">
                                        ...
                                        <form action="{{ route('registrar.programs.destroy', $program->id) }}" method="POST"
                                            ...
                                            @method('DELETE')
                                            <button type="submit"
                                                class="px-4 py-1.5 rounded-lg bg-rose-500/10 border border-rose-500/20 text-[10px] font-black uppercase tracking-widest text-rose-400 hover:bg-rose-500 hover:text-white transition-all">
                                                Delete
                                            </button>
                                        </form>
                                    </div>
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="py-20 text-center">
                                    <p class="text-xs font-black text-white/20 uppercase tracking-[0.4em]">No college
                                        programs found</p>
                                </td>
                            </tr>
                        @endforelse
        ...
        <section class="p-8 rounded-2xl bg-white/5 border border-white/10 shadow-2xl">
            <div class="flex justify-between items-center mb-10">
                <div>
                    ...
                    <p class="text-xs text-white/40 uppercase tracking-widest mt-2 font-bold">Academic & Tech-Voc Tracks
                    </p>
                </div>
                <a href="{{ route('registrar.programs.index', ['showModal' => 'true', 'type' => 'shs']) }}"
                    class="bg-emerald-600 hover:bg-emerald-500 text-white px-6 py-3 rounded-xl text-xs font-bold uppercase tracking-widest transition-all shadow-lg shadow-emerald-500/20 active:scale-95 flex items-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 4v16m8-8H4">
                        </path>
                    </svg>
                        ...
                        @forelse($strands as $strand)
                            ...
                            ($finalTrack === 'TVL' ? 'bg-orange-500/20 text-orange-400 border-orange-500/20' :
                                'bg-white/10 text-white/40 border-white/20') }}">
                                                ...
                                                <td class="py-5 px-8">
                                                    <span
                                                        class="text-emerald-400 font-bold font-mono text-xs">{{ $strand->course_code }}</span>
                                                </td>
                                                <td class="py-5 px-8">
                                                    <span
                                                        class="text-white font-bold group-hover:text-emerald-300 transition-colors uppercase tracking-tight">{{ $strand->course_name }}</span>
                                                </td>
                                                <td class="py-5 px-8">
                                                    <span
                                                        class="text-white/40 text-sm line-clamp-1 font-medium">{{ $strand->description ?: '—' }}</span>
                                                </td>
                                                <td class="py-5 px-8 text-right whitespace-nowrap">
                                                    <div class="flex justify-end gap-2">
                                                        ...
                                                        <form action="{{ route('registrar.programs.destroy', $strand->id) }}" method="POST"
                                                            style="display: inline;" onsubmit="return confirm('Are you sure?');">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit"
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="py-20 text-center">
                                    <p class="text-xs font-black text-white/20 uppercase tracking-[0.4em]">No SHS strands
                                        found</p>
                                </td>
                            </tr>
                        @endforelse
    ...
    @if($showModal)
        <div class="fixed inset-0 z-[100] flex items-center justify-center p-4 animate-in fade-in duration-300">
            <a href="{{ route('registrar.programs.index') }}"
                class="absolute inset-0 bg-[#060d1a]/95 backdrop-blur-2xl"></a>

            <div
                class="bg-[#0d1f3c] border border-white/10 w-full max-w-lg rounded-[32px] overflow-hidden shadow-[0_32px_120px_rgba(0,0,0,0.6)] relative z-10 transform animate-in zoom-in-95 duration-300">
                <div class="p-10">
                    <div class="mb-10 text-center">
                        @if($isEditMode)
                            <h3 class="text-xl font-bold text-white uppercase tracking-tight">Edit
                                {{ $isTypeProgram ? 'Program' : 'Strand' }}</h3>
                        @else
                            <h3 class="text-xl font-bold text-white uppercase tracking-tight">New
                                {{ $isTypeProgram ? 'Program' : 'Strand' }}</h3>
                        @endif
                        <p class="text-white/30 text-xs font-black uppercase tracking-[0.3em] mt-1">
                            {{ $isTypeProgram ? 'College Program' : 'Senior High' }} Information</p>
                    </div>

                    <form
                        action="{{ $isEditMode ? route('registrar.programs.update', $editingProgram->id) : route('registrar.programs.store') }}"
                        ...
                        @if(!$isTypeProgram)
                            <div class="space-y-2">
                                <label
                                    class="block text-xs font-black text-emerald-400 uppercase tracking-[0.2em] ml-1">Enrollment
                                    Track</label>
                                <select name="track"
                                    ...
                                    <option value="ACAD" class="bg-neutral-900 text-white" {{ old('track', $editingProgram ? $editingProgram->track : '') === 'ACAD' ? 'selected' : '' }}>Academic Track (ACAD)
                                    </option>
                                    <option value="TVL" class="bg-neutral-900 text-white" {{ old('track', $editingProgram ? $editingProgram->track : '') === 'TVL' ? 'selected' : '' }}>Technical Vocational
                                        Livelihood (TVL)</option>
                                </select>
                                @error('track') <span
                                    class="text-rose-500 text-xs font-bold uppercase tracking-tighter">{{ $message }}</span>
                                @enderror
                            </div>
                        @endif
                        ...
                        <div class="space-y-2">
                            <label
                                class="block text-xs font-black {{ $isTypeProgram ? 'text-blue-400' : 'text-emerald-400' }} uppercase tracking-[0.2em] ml-1">
                                {{ $isTypeProgram ? 'Program Code' : 'Strand Code' }}
                            </label>
                            <input type="text" name="course_code"
                                value="{{ old('course_code', $editingProgram ? $editingProgram->course_code : '') }}"
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="course_name"
                                value="{{ old('course_name', $editingProgram ? $editingProgram->course_name : '') }}"
                                class="w-full bg-white/5 text-white border border-white/10 py-4 px-6 rounded-xl outline-none placeholder-white/10 text-sm font-bold tracking-wider focus:border-{{ $isTypeProgram ? 'blue' : 'emerald' }}-500/50 transition-all shadow-inner uppercase"
                                placeholder="{{ $isTypeProgram ? 'e.g., BS Information Systems' : 'e.g., Science, Technology, Engineering and Mathematics' }}"
                                required>
                            @error('course_name') <span
                                class="text-rose-500 text-xs font-bold uppercase tracking-tighter">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="space-y-2">
                            <label
                                class="block text-xs font-black {{ $isTypeProgram ? 'text-blue-400' : 'text-emerald-400' }} uppercase tracking-[0.2em] ml-1">Description</label>
