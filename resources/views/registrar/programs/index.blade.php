        ...
        <div class="rounded-2xl border overflow-hidden relative"
                        ...
                        <tr class="hover:bg-indigo-500/[0.03] transition-all group">
                            ...
                            <td class="py-5 px-8 text-right whitespace-nowrap">
                                <div class="flex justify-end gap-2">
                                    <a href="{{ route('registrar.programs.index', ['edit_id' => $program->id]) }}" class="p-2 rounded-lg bg-white/5 border border-white/5 text-white/40 hover:text-indigo-400 hover:bg-indigo-500/10 transition-all">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                    </a>
                                    <form action="{{ route('registrar.programs.destroy', $program->id) }}" method="POST" class="inline" onsubmit="return confirm('Delete this program?')">
                                        @csrf
                                        @method('DELETE')
            ...
            @if($programs->hasPages())
                <div class="p-6 border-t border-white/5 bg-white/[0.01]">
                    {{ $programs->links('pagination.glass') }}
                </div>
            @endif
        </div>
