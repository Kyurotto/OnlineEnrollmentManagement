<x-layouts.cashier :title="'Manage Payments'">
    <div class="space-y-6 animate-in fade-in duration-500">
        ...
        <div class="rounded-2xl border overflow-hidden relative shadow-2xl shadow-black/40"
            ...
            <div class="px-8 py-6 border-b border-white/5 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-white/[0.01]">
                ...
                <a href="{{ route('cashier.payments.index', ['showModal' => 'true']) }}" class="bg-emerald-500 hover:bg-emerald-400 text-black text-xs font-black py-4 px-8 rounded-2xl uppercase tracking-[0.2em] transition-all shadow-xl shadow-emerald-500/20 active:scale-95 flex items-center gap-3">
            ...
            <div class="bg-white/[0.02] px-8 py-5 border-b border-white/5">
                <form action="{{ route('cashier.payments.index') }}" method="GET" class="flex flex-col lg:flex-row gap-4">
                    ...
                    <div class="flex flex-col sm:flex-row gap-3">
                        <div class="w-full sm:w-48 relative">
                            <select name="filter_course" onchange="this.form.submit()" class="w-full bg-white/5 border border-white/10 rounded-xl py-3 px-5 text-xs text-white/60 font-black uppercase tracking-widest focus:border-emerald-500/50 outline-none cursor-pointer appearance-none transition-all shadow-inner">
                                ...
                                @foreach(['BSIS-1', 'BSIS-2', 'BSIS-3', 'BSIS-4', 'DIT-1', 'DIT-2', 'DIT-3', 'DIT-4'] as $course)
                                    <option value="{{ $course }}" class="bg-[#0d1f3c]" {{ request('filter_course') == $course ? 'selected' : '' }}>{{ str_replace('-', ' ', $course) }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="w-full sm:w-40 relative">
                            <select name="status" onchange="this.form.submit()" class="w-full bg-white/5 border border-white/10 rounded-xl py-3 px-5 text-xs text-white/60 font-black uppercase tracking-widest focus:border-emerald-500/50 outline-none cursor-pointer appearance-none transition-all shadow-inner">
                                <option value="All statuses" class="bg-[#0d1f3c]">All Statuses</option>
                                @foreach(['Paid', 'Pending', 'Rejected'] as $s)
                                    <option value="{{ $s }}" class="bg-[#0d1f3c]" {{ request('status') == $s ? 'selected' : '' }}>{{ $s }}</option>
                                @endforeach
                            </select>
                            <div class="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-white/20">
                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M19 9l-7 7-7-7"></path></svg>
                        ...
                        <a href="{{ route('cashier.payments.index') }}" class="bg-white/5 border border-white/10 text-white/40 hover:text-white hover:bg-white/10 text-xs font-black uppercase tracking-widest px-6 flex items-center justify-center rounded-xl transition-all active:scale-95 shadow-lg">Reset</a>
                    </div>
                </form>
            </div>
            ...
            <div class="overflow-x-auto custom-scrollbar">
                <table class="w-full text-left font-medium text-sm">
                    <thead class="text-xs text-white/20 uppercase tracking-[0.2em] border-b border-white/5" style="background: rgba(255,255,255,0.02);">
                        <tr>
                            <th class="py-5 px-5 font-black">ID</th>
                        ...
                        <tr class="hover:bg-emerald-500/[0.03] transition-all group">
                            ...
                            <td class="py-5 px-5 text-right whitespace-nowrap">
                                <div class="flex justify-end gap-2 text-right">
                                    <a href="{{ route('cashier.payments.index', ['edit_id' => $payment->id]) }}" class="p-2.5 rounded-xl bg-white/5 border border-white/5 text-white/30 hover:text-emerald-400 hover:bg-emerald-500/10 hover:border-emerald-500/30 transition-all active:scale-95 shadow-lg group/btn">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                    </a>
                                    @if($payment->status !== 'Paid' && $payment->status !== 'Rejected')
                                        <form action="{{ route('cashier.payments.update_status', $payment->id) }}" method="POST" class="inline">
                                            @csrf
                                            @method('PATCH')
                                            <input type="hidden" name="status" value="Paid">
                                            <button type="submit" class="p-2.5 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 hover:bg-emerald-500 hover:text-black transition-all active:scale-95 shadow-lg shadow-emerald-500/10 group/paid" title="Mark as Paid">
                                        ...
                                        <form action="{{ route('cashier.payments.update_status', $payment->id) }}" method="POST" class="inline">
                                            ...
                                            <button type="submit" class="p-2.5 rounded-xl bg-rose-500/10 border border-rose-500/20 text-rose-400 hover:bg-rose-500 hover:text-white transition-all active:scale-95 shadow-lg shadow-rose-500/10 group/reject" title="Reject Payment">
        ...
        @php
            ...
            $editingPayment = $isEditMode ? \App\Models\Payment::find(request('edit_id')) : null;
        @endphp

        @if($showModal)
        <div class="fixed inset-0 z-[200] flex items-center justify-center p-4 animate-in fade-in duration-300">
            <a href="{{ route('cashier.payments.index') }}" class="absolute inset-0 bg-[#060d1a]/95 backdrop-blur-2xl transition-all"></a>
            ...
            <div class="bg-[#0d1f3c] border border-white/10 w-full max-w-lg rounded-[32px] overflow-hidden shadow-[0_32px_120px_rgba(0,0,0,0.6)] relative z-10 transform animate-in zoom-in-95 duration-300">
                <div class="p-10">
                    ...
                    <form action="{{ $isEditMode ? route('cashier.payments.update', $editingPayment->id) : route('cashier.payments.store') }}" method="POST" class="space-y-6">
                        @csrf
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="user_id" class="w-full bg-white/5 text-white border border-white/10 py-4 px-6 rounded-xl outline-none placeholder-white/10 text-sm font-bold tracking-wider focus:border-emerald-500/50 appearance-none transition-all cursor-pointer shadow-inner" required>
                                <option value="" class="bg-[#0d1f3c]">Select Student</option>
                                @foreach($students as $student)
                                    <option value="{{ $student->id }}" class="bg-[#0d1f3c] text-white" {{ old('user_id', $editingPayment ? $editingPayment->user_id : '') == $student->id ? 'selected' : '' }}>{{ $student->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div class="space-y-2">
                                ...
                                <input type="number" name="amount" step="0.01" value="{{ old('amount', $editingPayment ? $editingPayment->amount : '') }}"
                            ...
                            <div class="space-y-2 hover:translate-y-[-2px] transition-transform">
                                ...
                                <select name="payment_type" class="w-full bg-white/5 text-white border border-white/10 py-4 px-6 rounded-xl outline-none placeholder-white/10 text-sm font-bold tracking-wider focus:border-emerald-500/50 appearance-none transition-all cursor-pointer shadow-inner uppercase font-black tracking-widest" required>
                                    @foreach(['Cash', 'Gcash', 'PayMaya', 'Bank Transfer'] as $method)
                                        <option value="{{ $method }}" class="bg-[#0d1f3c]" {{ old('payment_type', $editingPayment ? $editingPayment->payment_method : 'Cash') == $method ? 'selected' : '' }}>{{ $method }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="reference_no" value="{{ old('reference_no', $editingPayment ? $editingPayment->transaction_id : '') }}"
