    ...
    <div class="fixed inset-0 w-screen h-screen bg-[#f6f5f7] flex justify-center items-center flex-col z-[9999] font-['Montserrat',sans-serif] overflow-y-auto py-10 px-4">
        ...
        <div class="bg-white rounded-[20px] shadow-[0_14px_28px_rgba(0,0,0,0.25),0_10px_10px_rgba(0,0,0,0.22)] relative overflow-hidden w-full max-w-[450px] p-10 flex flex-col items-center text-center">
            ...
            <form method="POST" action="{{ route('verification.send') }}" class="w-full">
                @csrf
                <button type="submit" class="w-full rounded-full border border-[#16a34a] bg-[#16a34a] text-white text-xs font-bold py-3 px-11 tracking-wider uppercase transition-transform duration-[80ms] ease-in active:scale-95 focus:outline-none">
            ...
            <form method="POST" action="{{ route('logout') }}" class="w-full mt-4">
                @csrf
                <button type="submit" class="bg-transparent border-none cursor-pointer block mt-2 text-[#333] text-sm no-underline hover:text-[#16a34a] transition-colors font-bold w-full text-center focus:outline-none">
                    {{ __('Log Out') }}
                </button>
            </form>
