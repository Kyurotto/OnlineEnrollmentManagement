                        ...
                        <tr wire:key="payment-{{ $payment->id }}" class="hover:bg-gray-50 transition group">
                            ...
                            <td class="py-4 px-6 text-right">
                                <div class="flex justify-end gap-2">
                                    <button wire:click="editPayment({{ $payment->id }})" class="px-3 py-1.5 text-xs font-semibold text-[#10B981] border border-[#10B981]/30 bg-[#10B981]/5 hover:bg-[#10B981]/10 rounded-lg transition-colors">
                                        Edit
                                    </button>
                                    @if($payment->status !== 'Paid')
                                    <button wire:click="markAsPaid({{ $payment->id }})" class="px-3 py-1.5 text-xs font-semibold text-white bg-[#10B981] hover:bg-[#059669] rounded-lg transition-colors shadow-sm">
                                        Mark as Paid
                                    </button>
                                    @endif
                                </div>
        ...
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-md border border-gray-200">
            ...
            <form wire:submit="savePayment" class="p-6 space-y-5">
                <div>
                    ...
                    <select wire:model="payment_user_id" class="w-full bg-white border border-gray-300 text-gray-900 rounded-lg p-3 text-sm focus:ring-2 focus:ring-[#10B981] focus:border-[#10B981] outline-none transition-all appearance-none cursor-pointer shadow-sm">
                        <option value="">Search or select a student...</option>
                        @foreach($students as $student)
                            <option value="{{ $student->id }}">{{ $student->name }} ({{ $student->email }})</option>
                        @endforeach
                    </select>
                    @error('payment_user_id') <span class="text-xs text-red-500 pt-1">{{ $message }}</span> @enderror
                </div>

                <div>
                    <label class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-2">Amount (₱)</label>
                    <input type="number" wire:model="payment_amount" step="0.01" class="w-full bg-white border border-gray-300 text-[#10B981] rounded-lg p-3 text-sm focus:ring-2 focus:ring-[#10B981] focus:border-[#10B981] outline-none font-bold transition-all placeholder-gray-400 shadow-sm" placeholder="0.00">
                    @error('payment_amount') <span class="text-xs text-red-500 pt-1">{{ $message }}</span> @enderror
                </div>
