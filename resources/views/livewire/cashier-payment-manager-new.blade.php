                    ...
                    @forelse($payments as $enrollment)
                        ...
                        <button wire:click="selectStudent({{ $enrollment->user_id }}, {{ $enrollment->id }})"
                            ...
                            <div class="flex items-start justify-between gap-2 mb-2">
                                <div
                                    class="text-xs font-black {{ in_array($enrollment->status, ['Dropped', 'Withdrawn']) ? 'text-rose-500' : 'text-blue-600' }} uppercase tracking-wider">
                                    {{ optional($student)->last_name }}, {{ optional($student)->first_name }}
                                </div>
                                <div class="flex items-center gap-1 flex-shrink-0">
                                    @if (in_array($enrollment->status, ['Dropped', 'Withdrawn']))
                                        <span
                                            class="text-[8px] font-black px-1.5 py-0.5 rounded bg-rose-50 border border-rose-100 text-rose-500 uppercase tracking-widest">
                                            {{ $enrollment->status }}
                                        </span>
                                    @endif
                        ...
                        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                            <div class="min-w-0">
                                <div class="flex items-center gap-3 mb-3 flex-wrap">
                                    ...
                                    @if ($enrollment && in_array($enrollment->status, ['Dropped', 'Withdrawn']))
                                        <span
                                            class="inline-flex items-center gap-1 px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest bg-rose-600 text-white shadow-lg">
                                            {{ $enrollment->status }}
                                        </span>
                                    @endif
                                </div>
                                <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest">
                                    ID: #{{ str_pad(optional($selectedStudent)->id, 3, '0', STR_PAD_LEFT) }} •
                                    {{ optional($enrollment)->year_level ?? 'N/A' }} •
                                    {{ optional($enrollment)->course_code ?? 'N/A' }}
                                </p>
                            ...
                            <div class="flex items-center gap-3">
                                <button wire:click="setPaymentMode"
                                    class="text-[10px] font-black py-2.5 px-6 rounded-full uppercase tracking-widest transition-all {{ !$isDropPayMode ? 'bg-blue-600 hover:bg-blue-500 text-white shadow-lg shadow-blue-500/30' : 'bg-white border border-slate-200 text-blue-600 hover:bg-slate-50' }}">
                                    Payment
                                </button>
                                <button wire:click="setDropPayMode"
                                    class="text-[10px] font-black py-2.5 px-6 rounded-full uppercase tracking-widest transition-all {{ $isDropPayMode ? 'bg-amber-500 hover:bg-amber-400 text-white shadow-lg shadow-amber-500/30' : 'bg-white border border-slate-200 text-slate-400 hover:bg-slate-50' }}">
                                    Drop Pay
                                </button>
                            </div>
                        </div>
                            ...
                            <div class="space-y-8 animate-in slide-in-from-bottom-2 duration-300">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                                    ...
                                    @else
                                        <div class="space-y-4">
                                            <h4
                                                class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-6">
                                    ...
                                    <div class="bg-blue-50/30 p-8 rounded-[32px] border border-blue-500/10">
                                        <h4
                                            class="text-[10px] font-black text-blue-600 uppercase tracking-[0.2em] mb-6">
                                            Discount Protocol</h4>
                                        @if ($uiDiscount > 0)
                                            <div class="space-y-4">
                                                <div class="flex items-center justify-between">
                                                    <span
                                                        class="font-black text-emerald-600 text-[10px] uppercase tracking-widest">✓
                                                        Discount Verified</span>
                                                    <span
                                                        class="font-black text-emerald-500 text-xs">₱{{ number_format($uiDiscount, 2) }}</span>
                                                </div>
                                                <button wire:click="removeDiscount"
                                                    class="w-full mt-4 bg-rose-50 hover:bg-rose-100 text-rose-600 border border-rose-100 font-black py-2.5 px-4 rounded-xl text-[9px] uppercase tracking-[0.2em] transition-all shadow-sm">
                                        ...
                                        @else
                                            <div class="space-y-5 w-full">
                                                <div class="space-y-2">
                                                    <label
                                                        class="block text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Discount
                                                        (%)</label>
                                                    <div class="relative group">
                                                        <span
                                                            class="absolute right-4 top-1/2 -translate-y-1/2 text-slate-300 font-black text-[10px]">%</span>
                                                ...
                                                <button wire:click="applyDiscount"
                                                    class="w-full bg-blue-600 hover:bg-blue-500 text-white font-black py-3.5 px-5 rounded-2xl uppercase tracking-[0.2em] transition-all text-[9px] shadow-lg shadow-blue-600/20 active:scale-95">
                                                    Apply Protocol
                                                </button>
                                            </div>
                                        @endif
                    ...
                    @if ($activeTab === 'assessment')
                        <div x-data="{ target: '{{ $actualRemainingPrevious > 0 ? 'previous' : 'current' }}' }"
                            ...
                            <div class="flex justify-between items-center mb-4 font-black">
                                <div class="flex items-center gap-3">
                                    <div
                                        class="w-8 h-8 {{ $isDropPayMode ? 'bg-amber-100 text-amber-600' : 'bg-blue-100 text-blue-600' }} rounded-xl flex items-center justify-center">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="{{ $isDropPayMode ? 'M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z' : 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4' }}">
                                            </path>
                                        </svg>
                                    </div>
                                    <div>
                                        <h4
                                            class="{{ $isDropPayMode ? 'text-amber-600' : 'text-blue-600' }} uppercase tracking-widest text-[9px]">
                                            Transaction Phase</h4>
                                        <p class="text-[10px] font-bold text-slate-400 mt-0.5">
                                            {{ $isDropPayMode ? 'Drop Protocol Active' : 'Standard Payment Processing' }}
                                        </p>
                                    </div>
                                </div>
                                <div class="text-right uppercase tracking-tight">
                                    <p class="text-[9px] text-slate-400">Amount Due</p>
                                    <p x-show="target === 'previous'" class="text-xl text-rose-600">₱
                                        {{ number_format($actualRemainingPrevious, 2) }}</p>
                                    <p x-show="target === 'current'" class="text-xl text-blue-600">₱
                                        {{ number_format($actualRemainingCurrent, 2) }}</p>
                                    <p x-show="target === 'combined'" class="text-xl text-blue-600">₱
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                                <div class="col-span-1">
                                    <label
                                        class="text-[9px] text-slate-500 font-black uppercase tracking-[0.2em] block mb-2">Payment
                                        Target</label>
                                    <div class="relative group">
                                        <select x-model="target"
                                            class="w-full bg-white text-blue-600 border-2 border-slate-100 py-3 px-3 rounded-xl outline-none text-[10px] font-black tracking-widest focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 appearance-none transition-all cursor-pointer shadow-sm uppercase">
                                        ...
                                        <div
                                            class="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-blue-600">
                                ...
                                <div class="col-span-1">
                                    <label
                                        class="text-[9px] text-slate-500 font-black uppercase tracking-[0.2em] block mb-2">Tendered
                                        Amount</label>
                                    <div class="relative">
                                        <span
                                            class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 font-black text-xs">₱</span>
                                ...
                                <div class="col-span-1">
                                    <label
                                        class="text-[9px] text-slate-500 font-black uppercase tracking-[0.2em] block mb-2">Reference</label>
                    ...
                    <div class="flex-grow flex flex-col items-center justify-center p-20 text-center space-y-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <p class="text-[10px] text-slate-300 font-bold uppercase tracking-widest">Select student
                                from directory to begin session</p>
                        </div>
                    </div>
                @endif
            </div>
            ...
            <div class="bg-white px-8 py-5 border-b border-blue-500/10">
                <div class="flex flex-col lg:flex-row gap-4">
                    ...
                    <div class="flex flex-col sm:flex-row gap-3">
                        ...
                        <a href="{{ route('cashier.payments.export', ['level' => $level ?? 'college']) }}"
                            class="bg-blue-600 hover:bg-blue-500 text-white text-xs font-black uppercase tracking-widest px-8 flex items-center justify-center rounded-xl transition-all active:scale-95 shadow-lg shadow-blue-600/20">
                    ...
                    <tbody class="divide-y divide-slate-100 bg-white">
                        @forelse($paymentLogs as $log)
                            <tr class="hover:bg-slate-50 transition-colors group">
                                <td class="py-5 px-8 font-black text-xs text-slate-700">
                                    #{{ str_pad($log->id, 4, '0', STR_PAD_LEFT) }}</td>
                                <td class="py-5 px-5">
                                    <div class="text-xs font-black text-black uppercase">
                                        {{ $log->user->last_name ?? 'N/A' }}, {{ $log->user->first_name ?? 'N/A' }}
                                    </div>
                                ...
                                <td class="py-5 px-5">
                                    <div class="text-xs font-black text-blue-600 uppercase">
                                        {{ $log->application->course_code ?? 'N/A' }}</div>
                                    <div class="text-[9px] text-slate-400 font-bold uppercase tracking-tight">
                                        {{ $log->application->year_level ?? 'N/A' }}</div>
                                </td>
                                <td class="py-5 px-5 text-xs font-bold text-slate-600 uppercase">
                                    {{ $log->payment_date ? \Carbon\Carbon::parse($log->payment_date)->format('M d, Y') : $log->created_at->format('M d, Y') }}
                                </td>
                                <td class="py-5 px-5 text-right font-black text-black text-xs">₱
                                    {{ number_format($log->amount, 2) }}</td>
                                <td class="py-5 px-8 text-center">
                                    <span
                                        class="px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest {{ $log->status === 'Paid' ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-rose-50 text-rose-600 border border-rose-100' }}">
                                        {{ $log->status }}
                                    </span>
                                </td>
                            </tr>
                            ...
                            <tr>
                                <td colspan="7" class="py-20 text-center bg-white">
                                    <p
                                        ...
                                        No transaction records found.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    @if ($showModal)
        <div class="fixed inset-0 z-[200] flex items-center justify-center p-4 animate-in fade-in duration-300">
            <button wire:click="closeModal"
                class="absolute inset-0 bg-blue-900/40 backdrop-blur-md transition-all cursor-default"></button>
            <div
                class="bg-white border border-blue-500/20 w-full max-w-lg rounded-[40px] overflow-hidden shadow-[0_32px_120px_rgba(30,58,138,0.2)] relative z-10 transform animate-in zoom-in-95 duration-300 p-12 text-center">
