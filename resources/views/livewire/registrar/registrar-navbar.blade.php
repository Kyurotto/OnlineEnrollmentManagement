<nav class="sticky top-0 z-20 shadow-lg border-b" wire:poll.3s
            ...
            <div class="flex items-center gap-8">
                    ...
                    <div>
                        ...
                        <span class="text-xs" style="color: #8ab4d8;">
                            @if ($currentRoute === 'registrar.dashboard')
                                Dashboard
                            @elseif($currentRoute === 'registrar.students.index')
                                Manage Students
                            @elseif($currentRoute === 'registrar.applications.index')
                                Manage Applications
                            @elseif($currentRoute === 'registrar.programs.index')
                                Program Mgmt
                            @elseif($currentRoute === 'registrar.academic_years.index')
                                Academic Years
                            @elseif($currentRoute === 'registrar.semesters.index')
                                Academic Terms
                            @elseif($currentRoute === 'registrar.sections.index')
                                Sectioning
                            @endif
                        </span>
                    </div>
                ...
                <div class="flex space-x-6 text-sm font-medium h-16" style="color: #8ab4d8;">
                    <a wire:navigate href="{{ route('registrar.dashboard') }}"
                        ...
                        style="{{ $currentRoute === 'registrar.dashboard' ? 'color: #a8d5f5; border-bottom: 2px solid #1a3a6e;' : '' }}"
                        onmouseover="this.style.color='#ffffff'"
                        onmouseout="this.style.color='{{ $currentRoute === 'registrar.dashboard' ? '#a8d5f5' : '#8ab4d8' }}'">Dashboard</a>
                </div>
            </div>
                            ...
                            <div class="max-h-64 overflow-y-auto custom-scrollbar p-2 space-y-2"
                                ...
                                @if (isset($dbNotifications) && count($dbNotifications) > 0)
                                    @foreach ($dbNotifications as $notification)
                                        <div class="block p-3 rounded-lg border transition group"
                                            ...
                                            <p
                                                class="text-[9px] mt-2 text-right text-white/20 font-bold uppercase tracking-tighter">
                                    ...
                                    @foreach ($enrollmentAlerts as $notif)
                                        <a wire:navigate href="{{ route('registrar.applications.index') }}"
                                            ...
                                            <p class="text-xs mt-1" style="color: #8ab4d8;">
                                                <span
                                                    class="font-medium text-white uppercase">{{ $notif->first_name ?? '' }}
                                                    {{ $notif->last_name ?? '' }}</span> status is <span
                                                    class="uppercase font-bold"
                                                    style="color: #a8d5f5;">{{ $notif->status }}</span>.
                                            </p>
                                ...
                                @if (
                                    (!isset($dbNotifications) || count($dbNotifications) === 0) &&
                                        (!isset($enrollmentAlerts) || count($enrollmentAlerts) === 0))
    ...
    <!-- Mobile Menu -->
    <div class="sm:hidden overflow-hidden transition-all duration-300 {{ $showMobileMenu ? 'max-h-[600px] opacity-100 border-t' : 'max-h-0 opacity-0 border-none' }}"
        style="background: rgba(6,13,26,0.98); border-color: rgba(26,58,110,0.4);">
        <div class="px-4 py-6 space-y-4">
            <a wire:navigate href="{{ route('registrar.dashboard') }}"
                class="block py-3 px-4 rounded-xl text-sm font-bold transition-all {{ $currentRoute === 'registrar.dashboard' ? 'text-white' : '' }}"
                style="{{ $currentRoute === 'registrar.dashboard' ? 'background: rgba(26,58,110,0.4); color: #a8d5f5;' : 'color: #8ab4d8;' }}">
                Dashboard
            </a>
            ...
            @if ($currentRoute === 'registrar.dashboard')
                <div class="pt-4 border-t border-white/5 font-bold">
                    <div class="px-4 py-2">
                        <p class="text-[10px] font-black uppercase tracking-widest" style="color: #4a6fa5;">Account
                            Manager</p>
                        <p class="text-sm font-bold text-white mt-1">Registrar</p>
                    </div>
                </div>
            @endif
        </div>
