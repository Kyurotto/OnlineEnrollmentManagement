<div x-data="{ modalOpen: false, selectedId: null }" x-init="$watch('modalOpen', value => {
    if (value === false) {
        selectedId = null;
    }
})" @keydown.escape.window="modalOpen = false; selectedId = null"
    @modal-reset.window="modalOpen = false; selectedId = null" wire:poll.15s
    ...
    <div class="bg-white rounded-[32px] border border-blue-500/10 shadow-xl overflow-hidden">
        <div
            class="p-8 md:p-10 border-b border-blue-500/10 bg-blue-50/30 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div class="flex-shrink-0">
                ...
                <p class="text-slate-400 text-[10px] font-black uppercase tracking-[0.2em] mt-1">
                    @if ($level === 'college')
                        College Application Review Pipeline
                    @elseif($level === 'shs')
                        Senior High School Application Review Pipeline
                    @else
                        Student Application Review Pipeline
                    @endif
                </p>
            </div>
            <div class="flex flex-col sm:flex-row items-center gap-4 w-full md:w-auto">
                <div class="relative group w-full sm:w-64">
                    ...
                    <div
                        class="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none opacity-40 group-focus-within:opacity-100 transition-opacity">
                ...
                <div class="w-full sm:w-44">
                    <select wire:model.live="status"
                        class="w-full bg-white border border-slate-200 rounded-2xl px-6 py-3.5 text-[11px] font-black uppercase tracking-widest text-black focus:outline-none focus:ring-2 focus:ring-blue-500/20 appearance-none cursor-pointer transition-all shadow-sm">
        ...
        <div class="overflow-x-auto custom-scrollbar" style="min-height: 450px;">
            <table class="min-w-max w-full text-left border-collapse font-bold bg-white">
                <thead
                    ...
                    <tr>
                        <th class="py-6 px-8 text-left cursor-pointer group/th hover:text-blue-600 transition-colors"
                            wire:click="sortBy('enrollments.id')">
                            <div class="flex items-center gap-2">
                                ID
                                <span
                                    class="transition-opacity {{ $sortField === 'enrollments.id' ? 'opacity-100' : 'opacity-0 group-hover/th:opacity-50' }}">
                        ...
                        <th class="py-6 px-8 text-left cursor-pointer group/th hover:text-blue-600 transition-colors"
                            wire:click="sortBy('users.last_name')">
                            <div class="flex items-center gap-2">
                                Full Name
                                <span
                                    class="transition-opacity {{ $sortField === 'users.last_name' ? 'opacity-100' : 'opacity-0 group-hover/th:opacity-50' }}">
                        ...
                        <th
                            class="py-6 px-8 text-left text-slate-400 cursor-pointer group/th hover:text-blue-600 transition-colors">
                            <div class="flex items-center gap-2">
                                Level
                            </div>
                        </th>
                        <th class="py-6 px-8 text-left cursor-pointer group/th hover:text-blue-600 transition-colors"
                            wire:click="sortBy('enrollments.course_code')">
                            <div class="flex items-center gap-2">
                                Program
                                <span
                                    class="transition-opacity {{ $sortField === 'enrollments.course_code' ? 'opacity-100' : 'opacity-0 group-hover/th:opacity-50' }}">
                        ...
                        <th class="py-6 px-8 text-left cursor-pointer group/th hover:text-blue-600 transition-colors"
                            wire:click="sortBy('enrollments.created_at')">
                            <div class="flex items-center gap-2">
                                Date
                                <span
                        ...
                        <th class="py-6 px-8 text-center cursor-pointer group/th hover:text-blue-600 transition-colors"
                ...
                <tbody class="text-xs divide-y divide-slate-100 bg-white">
                    @forelse($applications as $application)
                        <tr class="hover:bg-blue-50/30 transition-colors group">
                            <td class="py-6 px-8 text-slate-400 font-mono tracking-tighter whitespace-nowrap">
                            ...
                            <td class="py-6 px-8 text-slate-500 lowercase tracking-tight whitespace-nowrap">
                            ...
                            <td class="py-6 px-8 text-slate-500 font-medium tracking-tight whitespace-nowrap">
                                        ...
                                        <div x-show="showDropdown" @click.away="showDropdown = false" x-cloak
                                            ...
                                            <div x-show="classType === 'irregular'"
                                                ...
                                                <button type="button" @click="
                                                    ...
                                                    " :disabled="!classReason"
                                                    :class="!classReason ? 'opacity-40 cursor-not-allowed' : 'hover:bg-amber-600'"
                        ...
                        <tr>
                            <td colspan="8" class="py-24 text-center">
                                <div class="flex flex-col items-center opacity-30">
                                    <svg class="w-16 h-16 mb-6 text-slate-800" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                                    ...
                                    <span class="text-[10px] font-black uppercase tracking-[0.4em] text-slate-800">No
                                        applications found in the review pipeline</span>
                                </div>
                            </td>
                        </tr>
                    @endforelse
    ...
    <div x-show="modalOpen" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 scale-95"
        ...
        <div class="absolute inset-0 bg-blue-900/40 backdrop-blur-md" @click="modalOpen = false; selectedId = null">
        </div>

        <div class="bg-white w-full max-w-5xl rounded-[40px] shadow-[0_32px_120px_rgba(30,58,138,0.2)] border border-blue-500/20 overflow-hidden flex flex-col max-h-[95vh] relative z-10 transform transition-all duration-300"
            id="modalContent" wire:ignore>

            <div
                class="px-8 md:px-12 py-8 border-b border-blue-500/10 flex justify-between items-center gap-6 bg-blue-50/30">
                <div>
                    <span class="text-[9px] font-black text-blue-600 uppercase tracking-[0.4em] mb-1 block">Analysis
                        Protocol</span>
                    <h2 class="text-2xl font-black text-black uppercase tracking-tight" id="modalTitle">Application
                        Details</h2>
                </div>
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="relative group/voucherModal">
                        <button type="button"
                            class="px-5 py-3 rounded-lg bg-blue-600 text-white hover:bg-blue-500 transition-all text-[10px] font-black uppercase tracking-widest shadow-lg border border-blue-400 flex items-center gap-2 whitespace-nowrap">
                            <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="currentColor"
                                viewBox="0 0 24 24">
                                <path
                                    d="M20 4c1.1046 0 2 0.89543 2 2v3.1709c-0.0001 0.42374 -0.2675 0.80117 -0.667 0.9424C20.5549 10.3884 20 11.1308 20 12s0.5549 1.6116 1.333 1.8867c0.3995 0.1412 0.6669 0.5187 0.667 0.9424V18c0 1.1046 -0.8954 2 -2 2H4c-1.10457 0 -2 -0.8954 -2 -2v-3.1709c0.00008 -0.4237 0.26746 -0.8012 0.66699 -0.9424C3.44507 13.6116 4 12.8692 4 12s-0.55493 -1.6116 -1.33301 -1.8867C2.26746 9.97207 2.00008 9.59464 2 9.1709V6c0 -1.10457 0.89543 -2 2 -2zm-5.5 9c-0.8284 0 -1.5 0.6716 -1.5 1.5s0.6716 1.5 1.5 1.5 1.5 -0.6716 1.5 -1.5 -0.6716 -1.5 -1.5 -1.5m0.707 -4.20703c-0.3905 -0.39053 -1.0235 -0.39053 -1.414 0L8.79297 13.793c-0.39053 0.3905 -0.39053 1.0235 0 1.414 0.39052 0.3906 1.02354 0.3906 1.41403 0l5 -5c0.3906 -0.39049 0.3906 -1.02351 0 -1.41403M9.5 8C8.67157 8 8 8.67157 8 9.5c0 0.8284 0.67157 1.5 1.5 1.5 0.8284 0 1.5 -0.6716 1.5 -1.5 0 -0.82843 -0.6716 -1.5 -1.5 -1.5"
                                    stroke-width="1"></path>
                            </svg>
                            Voucher
                        </button>
                        <div
                            class="absolute right-0 mt-2 w-56 bg-white border border-slate-200 rounded-lg shadow-2xl opacity-0 invisible group-hover/voucherModal:opacity-100 group-hover/voucherModal:visible transition-all z-50 overflow-hidden">
                            <div id="voucherStatusModal" class="p-3 border-b border-slate-100 hidden">
                                ...
                                <div id="voucherBadgeModal"
                                    class="flex gap-1 items-center p-2 rounded text-[9px] font-black"></div>
                            </div>
                            <button onclick="applyVoucher('free_tuition')"
                                class="w-full text-left px-4 py-3 text-[9px] font-bold text-emerald-600 hover:bg-emerald-50 transition-colors uppercase tracking-widest">🟢
                                Free Tuition</button>
                            <button onclick="applyVoucher('discounted')"
                                class="w-full text-left px-4 py-3 text-[9px] font-bold text-blue-600 hover:bg-blue-50 transition-colors border-t border-slate-100 uppercase tracking-widest">🔵
                                Discounted</button>
                            <button onclick="removeVoucher()"
                                class="w-full text-left px-4 py-3 text-[9px] font-bold text-rose-600 hover:bg-rose-50 transition-colors border-t border-slate-100 uppercase tracking-widest">✕
                                Remove</button>
                        </div>
                    </div>

                    <button @click="modalOpen = false; selectedId = null"
                        class="p-3 rounded-xl bg-slate-100 text-slate-400 hover:text-black hover:bg-slate-200 transition-all border border-slate-200">
            ...
            <div class="p-8 md:p-12 overflow-y-auto custom-scrollbar flex-grow space-y-12">
                <div class="space-y-10">
                    ...
                    <div class="space-y-6">
                        <div class="flex items-center gap-3">
                            ...
                            <h3 class="text-[10px] font-black text-black uppercase tracking-[0.3em]">Program Details
                            </h3>
                        </div>
                        <div class="bg-blue-50 border border-blue-200 rounded-[32px] p-8">
                            <div class="space-y-4">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-8 pt-6 border-t border-blue-100">
                                    ...
                                    <div class="flex flex-col gap-1">
                                        <span
                                            class="text-[10px] font-black text-slate-500 uppercase tracking-widest">Status</span>
                    ...
                    <div class="space-y-6">
                        <div class="flex items-center gap-3">
                            <span class="w-1.5 h-1.5 rounded-full bg-blue-600"></span>
                            <h3 class="text-[10px] font-black text-black uppercase tracking-[0.3em]">Applicant Profile
                            </h3>
                        </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="flex flex-col gap-1 md:col-span-2">
                                    <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Full
                                        Name</span>
                                    <span class="text-lg font-black text-black uppercase tracking-wide"
                                        id="modalNameValue"></span>
                                </div>
                                <div class="flex flex-col gap-1">
                                    <span
                                        class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Extension</span>
                                    <span class="text-sm font-extrabold text-black uppercase"
                                        id="modalExtension"></span>
                                </div>
                                <div class="flex flex-col gap-1">
                                    <span
                                        class="text-[10px] font-black text-slate-400 uppercase tracking-widest">LRN</span>
                                    <span class="text-sm font-extrabold text-black uppercase" id="modalLrn"></span>
                                </div>
                            </div>

                            <div class="pt-5 border-t border-slate-100">
                                <h4 class="text-[10px] font-black text-blue-600 uppercase tracking-[0.2em] mb-4">
                                    Identity Details</h4>
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                    <div class="flex flex-col gap-1">
                                        <span
                                            class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Reference
                                            ID</span>
                                        <span class="text-sm font-extrabold text-blue-600 uppercase"
                                            id="modalAppId"></span>
                                    </div>
                                    <div class="flex flex-col gap-1">
                                        <span
                                            class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Applied
                                            On</span>
                                        <span class="text-sm font-extrabold text-black uppercase"
                                            id="modalSubmitted"></span>
                                    </div>
                                    <div class="flex flex-col gap-1">
                                        <span
                                            class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Gender</span>
                                        <span class="text-sm font-extrabold text-black uppercase"
                                            id="modalGender"></span>
                                    </div>
                                    <div class="flex flex-col gap-1">
                                    ...
                                    <div class="flex flex-col gap-1">
                                        <span
                                            class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Birthplace</span>
                                        <span class="text-sm font-extrabold text-black uppercase"
                                            id="modalBirthplace"></span>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                    <div class="flex flex-col gap-1 sm:col-span-2">
                                        <span
                                            ...
                                            Address</span>
                                        <span class="text-sm font-extrabold text-black lowercase"
                                            id="modalEmail"></span>
                                    </div>
                                    <div class="flex flex-col gap-1">
                                        <span
                                            class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Contact
                                            Number</span>
                                        <span class="text-sm font-extrabold text-black uppercase"
                                            id="modalContactNumber"></span>
                                    </div>
                                    <div class="flex flex-col gap-1">
                                        <span
                                            class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Facebook
                                            Account</span>
                                        <span class="text-sm font-extrabold text-black" id="modalFacebook"></span>
                                    </div>
                                    <div class="flex flex-col gap-1">
                                        <span
                                            class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Religion
                                            / Church</span>
                                        <span class="text-sm font-extrabold text-black uppercase"
                                            id="modalReligionChurch"></span>
                                    </div>
                                    <div class="flex flex-col gap-1">
                                        <span
                                            class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Junior
                                            High School</span>
                                        <span class="text-sm font-extrabold text-black uppercase"
                                            id="modalJuniorHigh"></span>
                                    </div>
                                    <div class="flex flex-col gap-1 sm:col-span-2">
                                        <span
                    ...
                    <div class="space-y-4">
                        <div class="flex items-center gap-3">
                            <span class="w-1.5 h-1.5 rounded-full bg-blue-600"></span>
                            <h3 class="text-[10px] font-black text-black uppercase tracking-[0.3em]">Guardian
                                Information</h3>
                        </div>
                        <div
                            class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-white border border-slate-200 rounded-[32px] p-8 shadow-sm">
                            <div class="flex flex-col gap-1">
                                <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Father's
                                    Name</span>
                                <span class="text-sm font-extrabold text-black uppercase" id="modalFather"></span>
                            </div>
                            <div class="flex flex-col gap-1">
                                <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Mother's
                                    Name</span>
                                <span class="text-sm font-extrabold text-black uppercase" id="modalMother"></span>
                            </div>
                            <div class="flex flex-col gap-1">
                                <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Guardian
                                    Name</span>
                                <span class="text-sm font-extrabold text-black uppercase" id="modalGuardian"></span>
                            </div>
                            <div class="flex flex-col gap-1">
                                <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Emergency
                                    Contact</span>
                    ...
                    <div class="space-y-6">
                        <div class="flex items-center gap-3">
                            <span class="w-1.5 h-1.5 rounded-full bg-blue-600"></span>
                            <h3 class="text-[10px] font-black text-black uppercase tracking-[0.3em]">Required Documents
                            </h3>
                        </div>
                ...
                <div class="space-y-6 pt-6 hidden" id="modalPromissorySection">
                    <div class="flex items-center gap-3">
                        ...
                        <h3 class="text-[10px] font-black text-black uppercase tracking-[0.3em]">Promissory Note &
                            Reason</h3>
                    </div>
                    <div
                        class="grid grid-cols-1 lg:grid-cols-3 gap-6 bg-amber-50 border border-amber-200 rounded-[32px] p-8">
                        <div id="modalPromissoryFile" class="lg:col-span-1">
                            {{-- Injected via JS --}}
                        </div>
                        <div class="lg:col-span-2 space-y-2">
                            <span class="text-[9px] font-black text-amber-600/60 uppercase tracking-widest">Student's
                                Explanation</span>
            ...
            <!-- Modal Footer -->
            <div
                class="px-8 md:px-12 py-8 border-t border-slate-100 bg-slate-50 flex flex-col md:flex-row justify-between items-center gap-6">
                <div class="flex flex-wrap items-center gap-4 w-full" id="actionButtons">
                    <button type="button" id="togglePhysicalBtn"
                        @click="$wire.togglePhysicalDocuments(currentApplicationId).then(() => { location.reload(); });"
                    ...
                    <button type="button" id="approveBtn" @click="
                            ...
                            if (cType === 'irregular' && !cReason) {
                                alert('Please provide a reason for the irregular classification before approving.');
                                return;
                            }
                            @this.approve(currentApplicationId, cType, cReason).then(() => { location.reload(); });
                        "
                        class="bg-emerald-600 hover:bg-emerald-700 text-white text-[10px] font-black py-4 px-10 rounded-2xl uppercase tracking-[0.2em] transition-all shadow-xl shadow-emerald-600/20 active:scale-95 shrink-0">
                        ✓ Approve
                    </button>
                    <button type="button" id="rejectBtn"
                        @click="if(confirm('Are you sure you want to reject this application?')) { @this.reject(currentApplicationId).then(() => { location.reload(); }); }"
    ...
    <script>
        ...
        function openModal(app, docMapping) {
            ...
            document.getElementById('modalSubmitted').innerText = new Date(app.created_at).toLocaleDateString('en-US', {
                month: 'short',
                day: 'numeric',
                year: 'numeric'
            });
            ...
            if (app.physical_documents_received) {
                ...
                toggleBtn.className =
                    'px-6 py-4 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] transition-all active:scale-95 shadow-xl bg-rose-500/10 text-rose-400 border border-rose-500/20 shrink-0';
            } else {
                toggleBtn.textContent = 'Done Hard Docs';
                toggleBtn.className =
                    'px-6 py-4 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] transition-all active:scale-95 shadow-xl bg-cyan-500 text-black shrink-0';
            }
            ...
            if (['pending', 'approved', 'enrolled', 'paid'].includes(status)) {
                ...
                if (app.physical_documents_received == 1) {
                    ...
                    docsBtn.className =
                        'text-[10px] font-black py-4 px-10 rounded-2xl uppercase tracking-[0.2em] transition-all shadow-xl active:scale-95 shrink-0 bg-rose-500/10 text-rose-400 border border-rose-500/20';
                } else {
                    docsBtn.textContent = 'Done Hard Docs';
                    docsBtn.className =
                        'text-[10px] font-black py-4 px-10 rounded-2xl uppercase tracking-[0.2em] transition-all shadow-xl active:scale-95 shrink-0 bg-emerald-500/10 text-emerald-400 border border-emerald-500/20';
                }
        ...
        function updateVoucherDisplay(voucherType) {
            ...
            if (voucherType) {
                ...
                if (voucherType === 'free_tuition') {
                    badge.className =
                        'flex items-center gap-2 p-2 rounded-lg bg-green-500/10 border border-green-500/20 text-green-400';
                    badge.textContent = '🟢 Free Tuition';
                } else {
                    badge.className =
                        'flex items-center gap-2 p-2 rounded-lg bg-yellow-500/10 border border-yellow-500/20 text-yellow-400';
