    ...
    <template x-if="showDropdown">
        <div class="absolute right-0 top-12 w-80 shadow-2xl rounded-xl z-50 overflow-hidden transform transition-all"
            ...
            <div class="px-4 py-3 border-b flex justify-between items-center"
                ...
                @if($unreadCount > 0)
                    <button type="button" wire:click="markAllAsRead" class="text-[10px] font-bold text-rose-400 hover:text-rose-300 uppercase tracking-widest">Clear All</button>
                @endif
            </div>
            <div class="max-h-64 overflow-y-auto p-2 space-y-2" style="background: rgba(6,13,26,0.6);">
                @if(count($notifications) > 0)
                    @foreach($notifications as $notification)
                        <div class="block p-3 rounded-lg border transition cursor-pointer hover:bg-white/5 bg-emerald-500/10 border-emerald-500/30">
