<div>
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="px-6 py-5 border-b border-gray-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                ...
                <button wire:click="openCreateModal" class="bg-[#10B981] hover:bg-[#059669] text-white text-xs font-bold py-2.5 px-5 rounded-lg shadow-md shadow-[#10B981]/10 uppercase tracking-wide transition flex items-center gap-2">
            ...
            <div class="bg-gray-50 px-6 py-4 border-b border-gray-200">
                <div class="flex flex-col sm:flex-row gap-3">
                    <div class="relative flex-grow">
                        ...
                        <input wire:model.live.debounce.300ms="search" type="text" class="pl-10 w-full bg-white border border-gray-300 rounded-lg p-2 text-sm text-gray-900 focus:ring-2 focus:ring-[#10B981] outline-none transition-all placeholder-gray-400 shadow-sm" placeholder="Search student name, email, or receipt #...">
                    </div>
                    ...
                    <button wire:click="resetFilters" class="bg-white border border-gray-300 text-gray-700 hover:text-gray-900 hover:bg-gray-50 text-sm font-medium px-4 py-2 rounded-lg transition text-center shadow-sm whitespace-nowrap">Reset</button>
                </div>
            </div>
                        ...
                        <tr wire:key="payment-{{ $payment->id }}" class="hover:bg-gray-50 transition group">
                            ...
                            <td class="py-4 px-6 text-right relative" x-data="{ open: false }">
                                <div class="relative inline-block text-left">
                                    ...
                                    <div x-show="open" style="display: none;" class="origin-top-right absolute right-0 mt-2 w-40 rounded-md shadow-lg bg-white border border-gray-200 ring-1 ring-black ring-opacity-5 z-50">
                                        <div class="py-1" role="menu">
                                            <button wire:click="editPayment({{ $payment->id }})" class="w-full text-left block px-4 py-2 text-xs text-[#10B981] font-semibold hover:bg-gray-50 bg-white" role="menuitem"><span class="flex items-center gap-2"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>Edit Details</span></button>
                                            @if($payment->status !== 'Paid')
                                            <button wire:click="markAsPaid({{ $payment->id }})" class="w-full text-left block px-4 py-2 text-xs text-gray-700 font-semibold hover:bg-gray-50 bg-white"><span class="flex items-center gap-2"><svg class="w-4 h-4 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>Mark as Paid</span></button>
                                            @endif
                                        </div>
                                    </div>
                                </div>
    ...
    <div class="fixed inset-0 bg-black/40 z-50 flex items-center justify-center backdrop-blur-sm p-4">
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-md border border-gray-200">
            ...
            <form wire:submit="savePayment" class="p-6 space-y-5">
                <div>
                    <label class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-2">Select Student</label>
                    <select wire:model="user_id" class="w-full bg-white border border-gray-300 text-gray-900 rounded-lg p-3 text-sm focus:ring-2 focus:ring-[#10B981] focus:border-[#10B981] outline-none transition-all appearance-none cursor-pointer shadow-sm">
                        <option value="">Search or select a student...</option>
                        @foreach($students as $student)
                            <option value="{{ $student->id }}">{{ $student->name }} ({{ $student->email }})</option>
                        @endforeach
                    </select>
                    @error('user_id') <span class="text-xs text-red-500 pt-1">{{ $message }}</span> @enderror
                </div>

                <div>
                    <label class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-2">Amount (₱)</label>
                    <input type="number" wire:model="amount" step="0.01" class="w-full bg-white border border-gray-300 text-[#10B981] rounded-lg p-3 text-sm focus:ring-2 focus:ring-[#10B981] focus:border-[#10B981] outline-none font-bold transition-all placeholder-gray-400 shadow-sm" placeholder="0.00">
                    @error('amount') <span class="text-xs text-red-500 pt-1">{{ $message }}</span> @enderror
                </div>
