            ...
            <div class="flex items-center gap-8">
                ...
                <div class="flex space-x-6 text-sm font-medium text-gray-600 h-16">
                    ...
                    <a wire:navigate href="{{ route('admin.applications.index') }}"
                        class="flex items-center hover:text-[#10B981] transition h-full {{ request()->routeIs('admin.applications.index') ? 'text-[#10B981] border-b-2 border-[#10B981]' : '' }}">Applications</a>
                    <a wire:navigate href="{{ route('admin.courses.index') }}"
                        class="flex items-center hover:text-[#10B981] transition h-full {{ request()->routeIs('admin.courses.index') ? 'text-[#10B981] border-b-2 border-[#10B981]' : '' }}">Courses</a>
                    <a wire:navigate href="{{ route('admin.payments.index') }}"
                        class="flex items-center hover:text-[#10B981] transition h-full {{ request()->routeIs('admin.payments.index') ? 'text-[#10B981] border-b-2 border-[#10B981]' : '' }}">Payments</a>
                    <a wire:navigate href="{{ route('admin.students.index') }}"
                        class="flex items-center hover:text-[#10B981] transition h-full {{ request()->routeIs('admin.students.index') ? 'text-[#10B981] border-b-2 border-[#10B981]' : '' }}">Students</a>
                </div>
            </div>
            ...
            <div class="flex items-center gap-6">
                ...
                <div class="relative cursor-pointer group mr-2" x-data="{ open: false }" @click.away="open = false">
                    <button @click="open = !open" class="relative p-2 text-gray-400 hover:text-gray-500 transition">
                        ...
                        @if($pendingCount > 0)
                        <span class="absolute top-1 flex h-3 w-3">
                    ...
                    <div x-show="open" x-transition.opacity class="absolute right-0 top-12 w-80 bg-white border border-gray-200 shadow-2xl rounded-xl z-50 overflow-hidden" style="display: none;">
                        <div class="px-4 py-3 bg-gray-50 border-b border-gray-200 flex justify-between items-center">
                            ...
                            @if($pendingCount > 0)
                            <span class="bg-red-100 text-red-600 text-[10px] font-bold px-2 py-0.5 rounded-full">{{ $pendingCount }} New</span>
                            @endif
                        </div>
                                    ...
                                    <a href="{{ route('admin.applications.index') }}"
                                        ...
                                        @if ($notif->status === 'Enrolled')
                                            ...
                                            <p class="text-xs text-[#A1A1AA] mt-1">
                                                <span class="font-bold text-gray-900 uppercase">{{ $notif->first_name }} {{ $notif->last_name }}</span> is now <span class="font-bold text-[#10B981]">PAID</span>.
                                            </p>
                                        @else
                                            <p class="text-sm font-bold text-gray-900 group-hover:text-[#10B981]">New Application</p>
                                            <p class="text-xs text-[#A1A1AA] mt-1">
                                                <span class="font-medium text-gray-900 uppercase">{{ $notif->first_name }} {{ $notif->last_name }}</span> applied for <span class="uppercase font-bold text-[#10B981]">{{ $notif->course_code }}</span>.
                                            </p>
                                        @endif
