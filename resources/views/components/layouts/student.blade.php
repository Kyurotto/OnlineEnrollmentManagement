...
<body x-data="{ sidebarOpen: localStorage.getItem('student_sidebar_open') === 'true', mobileOpen: false }" x-init="$watch('sidebarOpen', value => localStorage.setItem('student_sidebar_open', value))" class="text-slate-900 flex flex-row min-h-screen"
        ...
        <div x-show="sidebarOpen" x-transition:enter="transition ease-out duration-200"
            ...
            <div class="flex items-center gap-3 px-4 h-20 flex-shrink-0 overflow-hidden"
                ...
                <div class="text-white font-black p-2.5 rounded-xl text-sm uppercase flex-shrink-0 tracking-widest"
                    style="background: linear-gradient(135deg, #3b82f6, #2563eb); box-shadow: 0 4px 12px rgba(37,99,235,0.2);">
                    ST</div>
                <div class="whitespace-nowrap">
                    <div class="text-sm font-black leading-none text-slate-800 tracking-tight uppercase">Student Portal
                    </div>
                ...
                <nav class="space-y-1">
                    ...
                    <div x-data="{ open: {{ $isPaymentsActive ? 'true' : 'false' }} }" class="mx-3">
                        <button @click="open = !open"
                            ...
                            :class="(open || {{ $isPaymentsActive ? 'true' : 'false' }}) ? 'bg-indigo-50 text-indigo-600' : 'text-slate-500 hover:bg-indigo-50/50 hover:text-indigo-600'"
                            :style="(open || {{ $isPaymentsActive ? 'true' : 'false' }}) ? 'color: #4f46e5; background: #eef2ff;' : 'color: #64748b;'">
                            <div class="flex items-center gap-3">
                                <svg class="w-5 h-5 flex-shrink-0 transition-colors duration-200"
                                    :class="(open || {{ $isPaymentsActive ? 'true' : 'false' }}) ? 'text-indigo-600' : 'text-slate-400 group-hover:text-indigo-600'"
                            ...
                            <svg x-show="sidebarOpen" x-cloak class="w-4 h-4 transition-all duration-200"
                                :class="[ (open || {{ $isPaymentsActive ? 'true' : 'false' }}) ? 'text-indigo-600' : 'text-slate-400 group-hover:text-indigo-600', open ? 'rotate-180' : '' ]"
                                fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div x-show="open" x-cloak class="pl-8 pr-3 py-1.5 space-y-1 bg-slate-50 rounded-xl mt-1 border border-slate-100">
                            <a href="{{ route('student.payment') }}" class="block py-2 text-[13px] font-bold transition-all {{ request()->routeIs('student.payment') ? 'text-indigo-600' : 'text-slate-700 hover:text-slate-900' }}">Payments</a>
                        </div>
                    </div>

                    <div class="my-6 mx-4 border-t border-slate-50"></div>
                    <p x-show="sidebarOpen" x-cloak
                        class="text-[11px] font-black uppercase tracking-[0.2em] px-6 mb-3 whitespace-nowrap text-slate-300">
                        Management</p>
                    ...
                    @php
                        ...
                        if ($activeYear && $activeSemester) {
                            ...
                            $currentEnrollmentSidebar = \App\Models\Enrollment::where('user_id', $user->id)
                                ->where(function ($q) use ($activeYear, $activeSemester) {
                                    $q->where(function ($sub) use ($activeYear, $activeSemester) {
                                        $sub->where('semester_name', $activeSemester->name)->where(
                                            'academic_year_name',
                                            $activeYear->year_name,
                                        );
                                    })->orWhere(function ($sub) use ($activeYear, $activeSemester) {
                                        $sub->where('year_level', 'LIKE', '%' . $activeYear->year_name . '%')->where(
                                            'year_level',
                                            'LIKE',
                                            '%' . $activeSemester->name . '%',
                                        );
                            ...
                            $anyLatestEnrollment = \App\Models\Enrollment::where('user_id', $user->id)
                                ->latest()
                                ->first();
                            ...
                            $isOldStudentSidebar =
                                ...
                                ($allCount === 1 &&
                                    ...
                                    $activeYear &&
                                    $activeSemester &&
                                    (stripos((string) $anyLatestEnrollment->year_level, $activeYear->year_name) ===
                                        false ||
                                        stripos((string) $anyLatestEnrollment->year_level, $activeSemester->name) ===
                                            false));
    ...
    $isShellRecord =
        $currentEnrollmentSidebar &&
        (empty($currentEnrollmentSidebar->course_code) ||
            str_contains($currentEnrollmentSidebar->year_level, 'Returning Student'));
}
...
if (!$hasSubmittedSidebar) {
    ...
    // If they have a record, only allow clicking if it's a SHELL record that is CLEARED
                    ...
                    {{-- Enrollment Dropdown --}}
                    @php
                        $isEnrollmentActive = request()->routeIs('student.enrollment.*');
                    @endphp
                    <div class="relative group mx-3" x-data="{ open: {{ $isEnrollmentActive ? 'true' : 'false' }} }">
                        <button @click="open = !open"
                            class="w-full flex items-center justify-between px-3 py-3 rounded-xl text-[15px] font-bold transition-all duration-200 relative z-10
                            {{ $isEnrollmentActive ? 'bg-indigo-50 text-indigo-700' : 'text-slate-600 hover:bg-indigo-50/80 hover:text-indigo-600' }}">
                            <div class="flex items-center gap-3 w-full">
                                <svg class="w-5 h-5 flex-shrink-0 transition-colors {{ $isEnrollmentActive ? 'text-indigo-600' : 'text-slate-400 group-hover:text-indigo-600' }}"
                                    fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <svg x-show="sidebarOpen" x-cloak class="w-4 h-4 transition-all duration-200"
                                :class="[
                                    (open || {{ $isEnrollmentActive ? 'true' : 'false' }}) ? 'text-indigo-600' :
                        ...
                        <div x-show="open" x-cloak
                            ...
                            <a href="{{ route('student.enrollment.upload') }}"
                                class="block py-2 text-[13px] font-bold transition-all {{ request()->routeIs('student.enrollment.upload') ? 'text-indigo-600' : 'text-slate-700 hover:text-slate-900' }}">Documents</a>

                            @if ($canShowApplicationsLink)
                                <a href="{{ route('student.enrollment.create') }}"
                                    class="block py-2 text-[13px] font-bold transition-all {{ request()->routeIs('student.enrollment.create') ? 'text-indigo-600' : 'text-slate-700 hover:text-slate-900' }}">Applications</a>
                            @else
                                <div
                                    class="block py-2 text-[13px] font-bold text-slate-300 cursor-not-allowed opacity-50">
                                    Applications</div>
                            @endif
                            ...
                            @if ($canReviewSidebar)
                                <a href="{{ route('student.enrollment.review') }}"
                                    class="block py-2 text-[13px] font-bold transition-all {{ request()->routeIs('student.enrollment.review') ? 'text-indigo-600' : 'text-slate-700 hover:text-slate-900' }}">Review
                                    Application</a>
                            @else
                                <div
                                    class="block py-2 text-[13px] font-bold text-slate-300 cursor-not-allowed opacity-50">
                                    Review Application</div>
                            @endif
                        </div>
                    </div>
                </nav>
        ...
        <nav class="sticky top-0 z-20 shadow-sm border-b h-20 flex items-center bg-white/80 backdrop-blur-xl"
                    ...
                    <div x-data="{ open: false }" class="relative">
                        <button @click="open = !open" @click.away="open = false"
                            ...
                            <div class="text-right hidden sm:block">
                                <div class="text-[10px] text-blue-600 font-black uppercase tracking-widest">
                                    {{ Auth::user()->role }} Account</div>
            ...
            <div x-show="mobileOpen" x-transition:enter="transition ease-out duration-200"
                ...
                <div class="p-4 space-y-1">
                    ...
                    <div x-data="{ open: {{ $isPaymentsActive ? 'true' : 'false' }} }">
                        <button @click="open = !open"
                            class="w-full flex items-center justify-between px-5 py-3.5 rounded-2xl transition-colors duration-200 group
                                {{ $isPaymentsActive ? 'bg-indigo-50/80 text-indigo-700' : 'text-slate-600 hover:bg-indigo-50/80 hover:text-slate-900' }}">
                            <div class="flex items-center gap-4">
                                <svg class="w-5 h-5 flex-shrink-0 transition-colors duration-200 {{ $isPaymentsActive ? 'text-indigo-600' : 'text-slate-400 group-hover:text-indigo-600' }}"
                                    fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2z">
                                    </path>
                            ...
                            <svg class="w-4 h-4 transition-transform duration-200 {{ $isPaymentsActive ? 'text-indigo-600' : 'text-slate-400 group-hover:text-indigo-600' }}"
                        ...
                        <div x-show="open" x-collapse>
                            <div class="px-5 py-2 mt-1 space-y-1 bg-slate-50 rounded-2xl border border-slate-100">
                                <a href="{{ route('student.payment') }}"
                                    class="flex items-center gap-2 px-3 py-2 text-[13px] font-bold rounded-lg transition-colors {{ request()->routeIs('student.payment') ? 'text-indigo-700 bg-indigo-50/50' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100' }}">
                                    Manage Payments
                                </a>
                            </div>
                        </div>
                    ...
                    @php
                        $isStudentEnrollmentActive = request()->routeIs('student.enrollment.*');
                    @endphp
                    <div x-data="{ open: {{ $isStudentEnrollmentActive ? 'true' : 'false' }} }">
                        <button @click="open = !open"
                            class="w-full flex items-center justify-between px-5 py-3.5 rounded-2xl transition-colors duration-200 group
                                {{ $isStudentEnrollmentActive ? 'bg-indigo-50/80 text-indigo-700' : 'text-slate-600 hover:bg-indigo-50/80 hover:text-slate-900' }}">
                            <div class="flex items-center gap-4">
                                <svg class="w-5 h-5 flex-shrink-0 transition-colors duration-200 {{ $isStudentEnrollmentActive ? 'text-indigo-600' : 'text-slate-400 group-hover:text-indigo-600' }}"
                                    fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z">
                                    </path>
                                </svg>
                                <span class="font-bold text-[13px] uppercase tracking-wider">Enrollments</span>
                            </div>
                            <svg class="w-4 h-4 transition-transform duration-200 {{ $isStudentEnrollmentActive ? 'text-indigo-600' : 'text-slate-400 group-hover:text-indigo-600' }}"
                                :class="{ 'rotate-180': open }" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M19 9l-7 7-7-7"></path>
                        ...
                        <div x-show="open" x-collapse>
                            <div class="px-5 py-2 mt-1 space-y-1 bg-slate-50 rounded-2xl border border-slate-100">
                                <a href="{{ route('student.enrollment.upload') }}"
                                    class="flex items-center gap-2 px-3 py-2 text-[13px] font-bold rounded-lg transition-colors {{ request()->routeIs('student.enrollment.upload') ? 'text-indigo-700 bg-indigo-50/50' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100' }}">
                                    Documents
                                </a>

                                @if ($canShowApplicationsLink)
                                    <a href="{{ route('student.enrollment.create') }}"
                                        class="flex items-center gap-2 px-3 py-2 text-[13px] font-bold rounded-lg transition-colors {{ request()->routeIs('student.enrollment.create') ? 'text-indigo-700 bg-indigo-50/50' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100' }}">
                                        Applications
                                    </a>
                                @else
                                    <div
                                        class="flex items-center gap-2 px-3 py-2 text-[13px] font-bold text-slate-300 cursor-not-allowed opacity-50">
                                        Applications
                                    </div>
                                @endif
                                ...
                                @if ($canReviewSidebar)
                                    <a href="{{ route('student.enrollment.review') }}"
                                        class="flex items-center gap-2 px-3 py-2 text-[13px] font-bold rounded-lg transition-colors {{ request()->routeIs('student.enrollment.review') ? 'text-indigo-700 bg-indigo-50/50' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100' }}">
                                        Review Application
                                    </a>
                                @else
                                    <div
