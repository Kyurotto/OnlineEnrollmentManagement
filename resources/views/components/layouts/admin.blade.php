        ...
        <nav x-data="{ showDropdown: false, showMobileMenu: false }" class="sticky top-0 z-20 shadow-lg border-b"
                    ...
                    <div class="flex items-center gap-8">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <span class="text-xs" style="color: #8ab4d8;">Management Console</span>
                            </div>
                        </div>
                        <div class="flex space-x-6 text-sm font-medium h-16" style="color: #8ab4d8;">
                            <a href="{{ route('admin.dashboard') }}"
                                class="flex items-center transition h-full"
                                style="{{ request()->routeIs('admin.dashboard') ? 'color: #ffffff; border-bottom: 2px solid #a8d5f5;' : 'color: #8ab4d8;' }}"
                                onmouseover="this.style.color='#ffffff'"
                                onmouseout="this.style.color='{{ request()->routeIs('admin.dashboard') ? '#ffffff' : '#8ab4d8' }}'">Dashboard</a>
                        </div>
                    </div>
                                    ...
                                    <div class="max-h-64 overflow-y-auto custom-scrollbar p-2 space-y-2" style="background: rgba(6,13,26,0.6);">
                                        @if(isset($dbNotifications) && $dbNotifications->count() > 0)
                                            @foreach($dbNotifications as $notification)
                                                <div @click="fetch('{{ route('notifications.markAsRead', $notification->id) }}', { method: 'POST', headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}' } }).then(() => { window.location.href = '{{ route('admin.applications.index') }}' })"
            ...
            <div x-show="showMobileMenu" x-cloak class="sm:hidden overflow-hidden transition-all duration-300" style="background: rgba(6,13,26,0.98); border-top: 1px solid rgba(26,58,110,0.4);">
                <div class="px-4 py-6 space-y-4">
                    <a href="{{ route('admin.dashboard') }}"
                        class="block py-3 px-4 rounded-xl text-sm font-bold transition-all {{ request()->routeIs('admin.dashboard') ? 'bg-white/10 text-white' : 'text-white/80 hover:bg-white/5' }}">Dashboard</a>
                </div>
            </div>
        </nav>
    </div>
