        ...
        <div x-show="sidebarOpen" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 -translate-x-4" x-transition:enter-end="opacity-100 translate-x-0" class="flex flex-col flex-1">
            ...
            <div class="flex items-center gap-3 px-4 h-20 flex-shrink-0 overflow-hidden" style="border-bottom: 1px solid rgba(0,0,0,0.06);">
                <div class="text-white font-black p-2.5 rounded-xl text-sm uppercase flex-shrink-0 tracking-widest"
                     style="background: linear-gradient(135deg, #2563eb, #1d4ed8); box-shadow: 0 4px 12px rgba(37,99,235,0.2);">AD</div>
                <div class="whitespace-nowrap">
                    <div class="text-sm font-black leading-none text-slate-800 tracking-tight uppercase">Admin Portal</div>
                    <div class="text-[10px] mt-1 font-bold uppercase tracking-widest text-blue-600">Enterprise Control</div>
                </div>
            </div>
            ...
            <div class="py-6 flex-1 flex flex-col">
                ...
                <p class="text-[11px] font-black uppercase tracking-[0.2em] px-6 mb-3 whitespace-nowrap text-slate-400">Main Menu</p>
                ...
                {{-- Management Section --}}
                <p x-show="sidebarOpen" x-cloak class="text-[11px] font-black uppercase tracking-[0.2em] px-4 mb-3 whitespace-nowrap text-slate-300">Operational Nodes</p>
                ...
                <div class="space-y-1">
                    ...
                    <a href="{{ route('admin.students.index', ['level' => 'college']) }}"
                       class="flex items-center gap-3 mx-3 px-3 py-3 rounded-xl text-[15px] font-bold transition-all duration-200 relative"
                       style="{{ request()->routeIs('admin.students.*') && request('level') === 'college' ? 'background: #eff6ff; color: #2563eb;' : 'color: #64748b;' }}">
                    ...
                    <a href="{{ route('admin.students.index', ['level' => 'shs']) }}"
                       class="flex items-center gap-3 mx-3 px-3 py-3 rounded-xl text-[15px] font-bold transition-all duration-200 relative"
                       style="{{ request()->routeIs('admin.students.*') && request('level') === 'shs' ? 'background: #eff6ff; color: #2563eb;' : 'color: #64748b;' }}">
                    ...
                    <a href="{{ route('admin.payments.college') }}"
                       class="flex items-center gap-3 mx-3 px-3 py-3 rounded-xl text-[15px] font-bold transition-all duration-200 relative"
                       style="{{ request()->routeIs('admin.payments.college') ? 'background: #eff6ff; color: #2563eb;' : 'color: #64748b;' }}">
                        <svg class="w-5 h-5 flex-shrink-0 {{ request()->routeIs('admin.payments.college') ? 'text-blue-600' : 'text-slate-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"></path></svg>
                        <span x-show="sidebarOpen" x-cloak class="whitespace-nowrap">College Payments</span>
                    </a>

                    {{-- SHS Payments --}}
                    <a href="{{ route('admin.payments.shs') }}"
                       class="flex items-center gap-3 mx-3 px-3 py-3 rounded-xl text-[15px] font-bold transition-all duration-200 relative"
                       style="{{ request()->routeIs('admin.payments.shs') ? 'background: #eff6ff; color: #2563eb;' : 'color: #64748b;' }}">
                        <svg class="w-5 h-5 flex-shrink-0 {{ request()->routeIs('admin.payments.shs') ? 'text-blue-600' : 'text-slate-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"></path></svg>
                        <span x-show="sidebarOpen" x-cloak class="whitespace-nowrap">SHS Payments</span>
                    </a>

                    {{-- Applications for College --}}
                    <a href="{{ route('admin.applications.index', ['level' => 'college']) }}"
                       class="flex items-center gap-3 mx-3 px-3 py-3 rounded-xl text-[15px] font-bold transition-all duration-200 relative"
                       style="{{ request()->routeIs('admin.applications.*') && request('level') === 'college' ? 'background: #f5f3ff; color: #7c3aed;' : 'color: #64748b;' }}">
                        <svg class="w-5 h-5 flex-shrink-0 {{ request()->routeIs('admin.applications.*') && request('level') === 'college' ? 'text-purple-600' : 'text-slate-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"></path></svg>
                    ...
                    <a href="{{ route('admin.applications.index', ['level' => 'shs']) }}"
                       class="flex items-center gap-3 mx-3 px-3 py-3 rounded-xl text-[15px] font-bold transition-all duration-200 relative"
                       style="{{ request()->routeIs('admin.applications.*') && request('level') === 'shs' ? 'background: #f5f3ff; color: #7c3aed;' : 'color: #64748b;' }}">
                        <svg class="w-5 h-5 flex-shrink-0 {{ request()->routeIs('admin.applications.*') && request('level') === 'shs' ? 'text-purple-600' : 'text-slate-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"></path></svg>
                    ...
                    <a href="{{ route('admin.archives.index') }}"
                       ...
                       style="{{ request()->routeIs('admin.archives.*') ? 'background: #eff6ff; color: #2563eb;' : 'color: #64748b;' }}">
                        <svg class="w-5 h-5 flex-shrink-0 {{ request()->routeIs('admin.archives.*') ? 'text-blue-600' : 'text-slate-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <nav class="sticky top-0 z-20 shadow-sm border-b h-20 flex items-center bg-white/80 backdrop-blur-xl"
            ...
            <div class="w-full px-8 flex items-center justify-between">
                ...
                <button @click="sidebarOpen = !sidebarOpen" x-show="!sidebarOpen" class="hidden sm:flex text-slate-600 hover:bg-slate-100 p-2.5 rounded-xl transition-colors">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"></path></svg>
                </button>                <button @click="mobileOpen = !mobileOpen" class="sm:hidden text-slate-600 hover:bg-slate-100 p-2.5 rounded-xl transition-colors">
                    ...
                    <div x-data="{ open: false }" class="relative">
                        <button @click="open = !open" @click.away="open = false" class="flex items-center gap-3 bg-slate-50 hover:bg-slate-100 px-4 py-2 rounded-xl transition-all border border-slate-200/60">
                            <div class="text-right hidden sm:block">
                                <div class="text-[10px] text-blue-600 font-black uppercase tracking-widest">Active Admin</div>
                                <div class="text-sm font-black text-slate-800 uppercase tracking-tight">{{ auth()->user()->name ?? 'System Admin' }}</div>
                            </div>
            ...
            <div x-show="mobileOpen"
                ...
                <div class="p-6 space-y-3">
                    <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-4 px-5 py-4 rounded-2xl bg-blue-50 text-blue-600 font-black uppercase text-[11px] tracking-widest">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"></path></svg>
                        <span>Dashboard</span>
                    </a>
                    <a href="{{ route('admin.courses.index') }}" class="flex items-center gap-4 px-5 py-4 rounded-2xl text-slate-600 font-black uppercase text-[11px] tracking-widest hover:bg-slate-50 transition-colors">
                        <span>Course Management</span>
                    </a>
                    <a href="{{ route('admin.archives.index') }}" class="flex items-center gap-4 px-5 py-4 rounded-2xl text-slate-600 font-black uppercase text-[11px] tracking-widest hover:bg-slate-50 transition-colors">
