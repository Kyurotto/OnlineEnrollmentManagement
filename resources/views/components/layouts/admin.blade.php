            ...
            <nav class="space-y-0.5">
                ...
                <div class="mt-1 space-y-0.5">
                    ...
                    <a href="{{ route('admin.dropped.index') }}"
                       class="flex items-center gap-3 px-3 py-2 rounded-lg text-[15px] font-medium transition-all duration-200 relative"
                       style="{{ request()->routeIs('admin.dropped.*') ? 'background: rgba(239,68,68,0.12); color: #ffffff;' : 'color: #8ab4d8;' }}"
                       onmouseover="this.style.background='rgba(255,255,255,0.05)'; if(!this.dataset.active) this.style.color='#ffffff';"
                       onmouseout="this.style.background='{{ request()->routeIs('admin.dropped.*') ? 'rgba(239,68,68,0.12)' : 'transparent' }}'; if(!this.dataset.active) this.style.color='#8ab4d8';"
                       {{ request()->routeIs('admin.dropped.*') ? 'data-active=1' : '' }}>
                        @if(request()->routeIs('admin.dropped.*'))
