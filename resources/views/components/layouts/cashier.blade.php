...
<body x-data="{ sidebarOpen: localStorage.getItem('cashier_sidebar_open') === 'true', mobileOpen: false }" x-init="$watch('sidebarOpen', value => localStorage.setItem('cashier_sidebar_open', value))" class="text-slate-900 flex flex-row min-h-screen"
        ...
        <div x-show="sidebarOpen" x-transition:enter="transition ease-out duration-200"
            ...
            <div class="flex items-center gap-3 px-6 h-20 flex-shrink-0 overflow-hidden border-b"
                ...
                <div class="whitespace-nowrap transition-all duration-300">
                    <div class="text-sm font-black leading-none text-slate-800 uppercase tracking-tighter">Cashier Panel
                    </div>
                    <div class="text-[10px] mt-1 font-bold uppercase tracking-[0.2em] text-blue-600">Financial Control
                    </div>
                </div>
            </div>
            ...
            <div class="py-6 flex-1 flex flex-col">
                ...
                <nav class="space-y-1.5">
                    ...
                    <a href="{{ route('cashier.dashboard') }}"
                        ...
                        <svg class="w-5 h-5 flex-shrink-0 {{ request()->routeIs('cashier.dashboard') ? 'text-blue-600' : 'text-slate-400' }}"
                    ...
                    <p x-show="sidebarOpen" x-cloak
                        class="text-[10px] font-black uppercase tracking-[0.4em] px-6 mb-4 text-slate-300">Revenue
                        Streams</p>

                    {{-- Payments Dropdown --}}
                    @php
                        $isPaymentsActive = request()->routeIs('cashier.payments.*');
                    @endphp
                    <div x-data="{ open: {{ $isPaymentsActive ? 'true' : 'false' }} }" class="mx-4">
                        <button @click="open = !open"
                            class="w-full flex items-center justify-between px-4 py-3.5 rounded-2xl text-[14px] font-black uppercase tracking-widest transition-all duration-300 group"
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <span x-show="sidebarOpen" x-cloak
                                    class="whitespace-nowrap transition-colors">Payments</span>
                            </div>
                        ...
                        <div x-show="open" x-cloak
                            ...
                            <a href="{{ route('cashier.payments.shs') }}"
                                class="block py-2 text-[13px] font-bold transition-all {{ request()->routeIs('cashier.payments.shs') ? 'text-blue-600' : 'text-slate-700 hover:text-slate-900' }}">SHS
                                Payments</a>
                            <a href="{{ route('cashier.payments.college') }}"
                                class="block py-2 text-[13px] font-bold transition-all {{ request()->routeIs('cashier.payments.college') ? 'text-blue-600' : 'text-slate-700 hover:text-slate-900' }}">College
                                Payments</a>
                        </div>
                    </div>
                    ...
                    <div class="my-8 mx-6 border-t border-slate-100"></div>
                    <p x-show="sidebarOpen" x-cloak
                        class="text-[10px] font-black uppercase tracking-[0.4em] px-6 mb-4 text-slate-300">Audit &
                        Config</p>

                    {{-- Assessment Dropdown --}}
                    @php
                        $isAssessmentActive = request()->routeIs('cashier.assessment.*');
                    @endphp
                    <div x-data="{ open: {{ $isAssessmentActive ? 'true' : 'false' }} }" class="mx-4">
                        <button @click="open = !open"
                            class="w-full flex items-center justify-between px-4 py-3.5 rounded-2xl text-[14px] font-black uppercase tracking-widest transition-all duration-300 group"
                            :class="(open || {{ $isAssessmentActive ? 'true' : 'false' }}) ? 'bg-blue-50 text-blue-600' :
                            'text-slate-600 hover:bg-blue-50/50 hover:text-blue-600'"
                            :style="(open || {{ $isAssessmentActive ? 'true' : 'false' }}) ?
                            'color: #2563eb; background: #eff6ff;' : 'color: #64748b;'">
                            <div class="flex items-center gap-3">
                                <svg class="w-5 h-5 flex-shrink-0 transition-colors duration-200"
                                    :class="(open || {{ $isAssessmentActive ? 'true' : 'false' }}) ? 'text-blue-600' :
                                    'text-slate-400 group-hover:text-blue-600'"
                                    fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <span x-show="sidebarOpen" x-cloak
                                    class="whitespace-nowrap transition-colors">Assessments</span>
                            </div>
                            <svg x-show="sidebarOpen" x-cloak class="w-4 h-4 transition-all duration-200"
                                :class="[
                                    (open || {{ $isAssessmentActive ? 'true' : 'false' }}) ? 'text-blue-600' :
                                    'text-slate-400 group-hover:text-blue-600',
                                    open ? 'rotate-180' : ''
                                ]"
                                fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div x-show="open" x-cloak
                            ...
                            <a href="{{ route('cashier.assessment.shs') }}"
                                class="block py-2 text-[13px] font-bold transition-all {{ request()->routeIs('cashier.assessment.shs') ? 'text-blue-600' : 'text-slate-700 hover:text-slate-900' }}">SHS
                                Assessment</a>
                            <a href="{{ route('cashier.assessment.college') }}"
                                class="block py-2 text-[13px] font-bold transition-all {{ request()->routeIs('cashier.assessment.college') ? 'text-blue-600' : 'text-slate-700 hover:text-slate-900' }}">College
                                Assessment</a>
                        </div>
                    </div>
        ...
        <nav class="sticky top-0 z-20 shadow-sm border-b h-20 flex items-center bg-white/80 backdrop-blur-xl"
                        ...
                        <button @click="open = !open" @click.away="open = false"
                            ...
                            <div class="text-right hidden sm:block">
                                ...
                                <div class="text-sm font-black text-slate-800 uppercase tracking-tight">
                                    {{ auth()->user()->name ?? 'System Cashier' }}</div>
                            </div>
            ...
            <div x-show="mobileOpen" x-transition:enter="transition ease-out duration-200"
                ...
                <div class="p-4 space-y-1">
                    ...
                    <p class="text-[10px] font-black uppercase tracking-[0.2em] px-4 py-2 text-slate-300">Revenue
                        Streams</p>
                    ...
                    <div x-data="{ open: {{ $isPaymentsActive ? 'true' : 'false' }} }">
                        ...
                        <div x-show="open" x-collapse>
                            <div class="px-5 py-2 mt-1 space-y-1 bg-slate-50 rounded-2xl border border-slate-100">
                                <a href="{{ route('cashier.payments.shs') }}"
                                    class="flex items-center gap-2 px-3 py-2 text-[13px] font-bold rounded-lg transition-colors {{ request()->routeIs('cashier.payments.shs') ? 'text-blue-700 bg-blue-50/50' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100' }}">
                                    SHS Payments
                                </a>
                                <a href="{{ route('cashier.payments.college') }}"
                                    class="flex items-center gap-2 px-3 py-2 text-[13px] font-bold rounded-lg transition-colors {{ request()->routeIs('cashier.payments.college') ? 'text-blue-700 bg-blue-50/50' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100' }}">
                                    College Payments
                                </a>
                            </div>
                        </div>
                    ...
                    <div class="my-4 border-t border-slate-50"></div>
                    <p class="text-[10px] font-black uppercase tracking-[0.2em] px-4 py-2 text-slate-300">Audit &
                        Config</p>
                    ...
                    @php
                        $isAssessmentActive = request()->routeIs('cashier.assessment.*');
                    @endphp
                    <div x-data="{ open: {{ $isAssessmentActive ? 'true' : 'false' }} }">
                        <button @click="open = !open"
                            class="w-full flex items-center justify-between px-5 py-3.5 rounded-2xl transition-colors duration-200 group
                                {{ $isAssessmentActive ? 'bg-blue-50/80 text-blue-700' : 'text-slate-600 hover:bg-blue-50/80 hover:text-slate-900' }}">
                            <div class="flex items-center gap-4">
                                <svg class="w-5 h-5 flex-shrink-0 transition-colors duration-200 {{ $isAssessmentActive ? 'text-blue-600' : 'text-slate-400 group-hover:text-blue-600' }}"
                                    fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z">
                                    </path>
                                </svg>
                                <span class="font-bold text-[13px] uppercase tracking-wider">Assessments</span>
                            </div>
                            <svg class="w-4 h-4 transition-transform duration-200 {{ $isAssessmentActive ? 'text-blue-600' : 'text-slate-400 group-hover:text-blue-600' }}"
                                :class="{ 'rotate-180': open }" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M19 9l-7 7-7-7"></path>
                        ...
                        <div x-show="open" x-collapse>
                            <div class="px-5 py-2 mt-1 space-y-1 bg-slate-50 rounded-2xl border border-slate-100">
                                <a href="{{ route('cashier.assessment.shs') }}"
                                    class="flex items-center gap-2 px-3 py-2 text-[13px] font-bold rounded-lg transition-colors {{ request()->routeIs('cashier.assessment.shs') ? 'text-blue-700 bg-blue-50/50' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100' }}">
                                    SHS Assessment
                                </a>
                                <a href="{{ route('cashier.assessment.college') }}"
                                    class="flex items-center gap-2 px-3 py-2 text-[13px] font-bold rounded-lg transition-colors {{ request()->routeIs('cashier.assessment.college') ? 'text-blue-700 bg-blue-50/50' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100' }}">
                                    College Assessment
                                </a>
                            </div>
                        </div>
        ...
        <footer class="py-12 border-t mt-auto"
            ...
            <div class="text-center">
                <p class="text-[10px] font-black text-slate-300 uppercase tracking-[0.4em]">
                    &copy; 2026 Your Institution &mdash; <span class="text-blue-600">Cashier Panel</span>
                </p>
